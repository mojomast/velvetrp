...
export function runM15Mutation<T extends object>(db:DatabaseDriver.Database,deps:M15Dependencies,assertMutation:()=>void,values:{
  ...
  return db.transaction(()=>{
    ...
    // public v25 command vocabulary, so this IMMEDIATE transaction advances it
    // exactly once with the initiating command.
    ...
    for(const other of [...new Set(values.additionalActorIds??[])].filter(id=>id!==values.actorId)){
      // The counterpart is deliberately not authorized here: transfers and
      // trade proposals need recipient consent only at acceptance.
