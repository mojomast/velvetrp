  ...
  return {
    ...
    executeAdventureCheckCandidate(principalIdInput, input) {
      ...
      return db.transaction(() => {
        ...
        const projected=JSON.parse(candidate.projection_json) as {version:string;candidates:ProviderSafeAdventureCheckCandidate[]};
        ...
        if (!response || response.round_number !== input.round || sha(response.request_json) !== response.request_digest || sha(response.response_json) !== response.response_digest
          ...
          || stripCandidateLabels(requestedProjection).candidates.some((requested:any)=>!projected.candidates.some((stored)=>canonicalAgentJson(stored as never)===canonicalAgentJson(requested as never)))
        ...
        const resultJson = canonicalAgentJson(receipt as never), commandId = `check-command:${sha(`${turnId}\0${selection.candidateId}`).slice(0, 48)}`;
