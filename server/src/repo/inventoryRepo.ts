  ...
  const mutate=(principal:string,input:InventoryCommand)=>{const command=inventoryCommandSchema.parse(input),changed=[`inventory:${command.actorId}`];if('recipientActorId'in command)changed.push(`inventory:${command.recipientActorId}`);
    return runM15Mutation(db,deps,assertMutation,{principal,campaignId:command.campaignId,actorId:command.actorId,family:'inventory',type:command.type,expectedRevision:command.expectedRevision,idempotencyKey:command.idempotencyKey,request:command,changedKeys:changed,additionalActorIds:command.type==='transfer_inventory_item'?[command.recipientActorId]:[],apply:(_revision,now)=>{
      ...
      if(command.type==='add_inventory_item'){
       ...
       else if(command.type==='equip_inventory_item'){if(!entry)throw new ActorResourceConflictError('inventory entry unavailable');const definition=itemDefinition(command.campaignId,{packId:entry.item_pack_id,packVersion:entry.item_pack_version,definitionId:entry.item_definition_id});if(!definition)throw new ActorResourceConflictError('inventory item definition is not pinned');if(definition.mechanics.slot===null)throw new InventoryBindingError('inventory item is not equipment');if(command.slot!==definition.mechanics.slot)throw new InventorySlotConflictError('inventory item requires its declared slot');if(entry.equipped)throw new InventorySlotConflictError('entry is already equipped');if(db.prepare("SELECT 1 FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND equipped=1 AND slot_key=?").get(command.campaignId,command.actorId,command.slot))throw new InventorySlotConflictError('equipment slot is occupied');db.prepare("UPDATE rpg_inventory_entries_v25 SET equipped=1,slot_key=? WHERE entry_id=?").run(command.slot,entry.entry_id);}
