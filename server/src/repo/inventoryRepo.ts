...
import { actorInventorySchema, inventoryCommandSchema, itemCatalogDefinitionSchema, resourceIdSchema, type ActorInventory, type InventoryCommand } from "@velvet/contracts";
...
export type ActorScopedInventoryCommand=
  |{kind:"equip";entryId:string;slot:string;hand?:"main"|"off"|undefined;grip?:"one-handed"|"two-handed"|undefined;expectedRevision:number;idempotencyKey:string}
...
export function createInventoryRepository(db:DatabaseDriver.Database,deps:M15Dependencies,assertMutation:()=>void):InventoryRepository {
  ...
  const read=(principal:string,campaign:string,actor:string):ActorInventory|null=>{resourceIdSchema.parse(principal);resourceIdSchema.parse(campaign);resourceIdSchema.parse(actor);if(!m15Authorized(db,principal,campaign,actor))return null;
    ...
    return actorInventorySchema.parse({campaignId:campaign,actorId:actor,inventory:{capacity:capacity(campaign,actor),items},equipment:rows.filter(row=>row.equipped).map(row=>({slot:row.slot_key,entryId:row.entry_id,...(row.hand_key?{hand:row.hand_key}:{}),...(row.grip_key?{grip:row.grip_key}:{})}))});};
  ...
  const itemDefinition=(campaign:string,item:{packId:string;packVersion:string;definitionId:string})=>{
    const row=db.prepare(`SELECT definition.definition_json FROM campaign_catalog_current_pins pin JOIN rpg_catalog_definitions definition
      ...
      .get(campaign,item.packId,item.packVersion,item.definitionId) as {definition_json:string}|undefined;
    return row?itemCatalogDefinitionSchema.parse(JSON.parse(row.definition_json)):null;
  };
  ...
  const mutate=(principal:string,input:InventoryCommand)=>{const command=inventoryCommandSchema.parse(input),changed=[`inventory:${command.actorId}`];if('recipientActorId'in command)changed.push(`inventory:${command.recipientActorId}`);
    return runM15Mutation(db,deps,assertMutation,{principal,campaignId:command.campaignId,actorId:command.actorId,family:'inventory',type:command.type,expectedRevision:command.expectedRevision,idempotencyKey:command.idempotencyKey,request:command,changedKeys:changed,additionalActorIds:command.type==='transfer_inventory_item'?[command.recipientActorId]:[],apply:(_revision,now)=>{
      if(!m15Authorized(db,principal,command.campaignId,command.actorId))throw new InventoryAuthorizationError('inventory unavailable');
       ...
       if(command.type==='equip_inventory_item'&&entry&&entry.quantity!==1)throw new InventorySlotConflictError('stackable items cannot be equipped');
      const remove=(quantity:number)=>{if(!entry)throw new ActorResourceConflictError('inventory entry is unavailable');if(entry.equipped)throw new InventoryBindingError('equipped item must be unequipped first');if(quantity>entry.quantity)throw new ActorResourceNegativeError('inventory quantity cannot become negative');if(quantity===entry.quantity)db.prepare("DELETE FROM rpg_inventory_entries_v25 WHERE entry_id=?").run(entry.entry_id);else db.prepare("UPDATE rpg_inventory_entries_v25 SET quantity=quantity-? WHERE entry_id=?").run(quantity,entry.entry_id);};
       if(command.type==='add_inventory_item'){
         ...
         const addedDefinition=itemDefinition(command.campaignId,command.item.item);
         if(!addedDefinition || addedDefinition.mechanics.stackable !== (command.item.kind==='stackable'))throw new InventoryBindingError('inventory mode does not match the pinned item');
         if(command.item.kind==='stackable'){const same=db.prepare(`SELECT entry_id FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND entry_mode='stackable' AND item_pack_id=? AND item_pack_version=? AND item_definition_id=?`).get(command.campaignId,command.actorId,command.item.item.packId,command.item.item.packVersion,command.item.item.definitionId) as any;
           if(same)db.prepare("UPDATE rpg_inventory_entries_v25 SET quantity=quantity+? WHERE entry_id=?").run(command.item.quantity,same.entry_id);else {if((read(principal,command.campaignId,command.actorId)?.inventory.items.length??0)>=capacity(command.campaignId,command.actorId))throw new InventoryCapacityError('inventory is full');db.prepare("INSERT INTO rpg_inventory_entries_v25(entry_id,campaign_id,actor_id,item_pack_id,item_pack_version,item_kind,item_definition_id,entry_mode,quantity,instance_key,slot_key,hand_key,grip_key,equipped,created_at) VALUES(?,?,?,?,?, 'item',?,'stackable',?,NULL,NULL,NULL,NULL,0,?)").run(command.item.entryId,command.campaignId,command.actorId,command.item.item.packId,command.item.item.packVersion,command.item.item.definitionId,command.item.quantity,now);}}
         else {if(!itemPinned(command.campaignId,command.item.item))throw new ActorResourceConflictError('item is not pinned by campaign catalog');if((read(principal,command.campaignId,command.actorId)?.inventory.items.length??0)>=capacity(command.campaignId,command.actorId))throw new InventoryCapacityError('inventory is full');db.prepare("INSERT INTO rpg_inventory_entries_v25(entry_id,campaign_id,actor_id,item_pack_id,item_pack_version,item_kind,item_definition_id,entry_mode,quantity,instance_key,slot_key,hand_key,grip_key,equipped,created_at) VALUES(?,?,?,?,?,'item',?,'instanced',1,?,NULL,NULL,NULL,0,?)").run(command.item.entryId,command.campaignId,command.actorId,command.item.item.packId,command.item.item.packVersion,command.item.item.definitionId,command.item.entryId,now);}
      } else if(command.type==='remove_inventory_item') remove(command.quantity??entry?.quantity??0);
       else if(command.type==='consume_inventory_item'||command.type==='drop_inventory_item') {if(!entry||entry.item_pack_id!==command.item.packId||entry.item_pack_version!==command.item.packVersion||entry.item_definition_id!==command.item.definitionId)throw new ActorResourceConflictError('inventory item identity differs');if(command.type==='consume_inventory_item'&&itemDefinition(command.campaignId,command.item)?.mechanics.category!=='consumable')throw new ActorResourceConflictError('inventory item is not consumable');remove(command.quantity);}
        else if(command.type==='equip_inventory_item'){if(!entry)throw new ActorResourceConflictError('inventory entry unavailable');const definition=itemDefinition(command.campaignId,{packId:entry.item_pack_id,packVersion:entry.item_pack_version,definitionId:entry.item_definition_id});if(!definition)throw new ActorResourceConflictError('inventory item definition is not pinned');if(definition.mechanics.slot===null)throw new InventoryBindingError('inventory item is not equipment');const profile=definition.mechanics.engineDetails?.equipmentProfile;if(entry.quantity!==1)throw new InventoryBindingError('stackable items cannot be equipped');if(command.slot!==definition.mechanics.slot)throw new InventorySlotConflictError('inventory item requires its declared slot');if(entry.equipped)throw new InventorySlotConflictError('entry is already equipped');if(command.slot!=='hand'&&(command.hand||command.grip))throw new InventoryBindingError('hand state requires the hand slot');if(profile?.kind==='armor'&&command.grip)throw new InventoryBindingError('armor does not have a weapon grip');const twoHanded=profile?.kind==='weapon'&&profile.properties.some(property=>property.property==='two-handed');if(twoHanded&&(command.hand!=="main"||command.grip!=="two-handed"))throw new InventoryBindingError('two-handed weapons require both hands');if(!twoHanded&&command.grip==='two-handed')throw new InventoryBindingError('item does not support a two-handed grip');if(command.grip&& !command.hand)throw new InventoryBindingError('grip requires an explicit hand');if(command.hand&&db.prepare("SELECT 1 FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND equipped=1 AND hand_key=?").get(command.campaignId,command.actorId,command.hand))throw new InventorySlotConflictError('equipment hand is occupied');if(twoHanded&&db.prepare("SELECT 1 FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND equipped=1 AND hand_key IN ('main','off')").get(command.campaignId,command.actorId))throw new InventorySlotConflictError('both hands must be free');if(command.slot!=='hand'&&db.prepare("SELECT 1 FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND equipped=1 AND slot_key=?").get(command.campaignId,command.actorId,command.slot))throw new InventorySlotConflictError('equipment slot is occupied');db.prepare("UPDATE rpg_inventory_entries_v25 SET equipped=1,slot_key=?,hand_key=?,grip_key=? WHERE entry_id=?").run(command.slot,command.hand??null,command.grip??(twoHanded?'two-handed':null),entry.entry_id);}
       else if(command.type==='unequip_inventory_item'){const equipped=db.prepare("SELECT entry_id FROM rpg_inventory_entries_v25 WHERE campaign_id=? AND actor_id=? AND equipped=1 AND slot_key=?").get(command.campaignId,command.actorId,command.slot)as any;if(!equipped)throw new InventorySlotConflictError('equipment slot is empty');db.prepare("UPDATE rpg_inventory_entries_v25 SET equipped=0,slot_key=NULL,hand_key=NULL,grip_key=NULL WHERE entry_id=?").run(equipped.entry_id);}
         ...
         if(entry.entry_mode==='instanced') {
           ...
           // Ownership changes; the durable instance identity does not.
            db.prepare("UPDATE rpg_inventory_entries_v25 SET actor_id=?,slot_key=NULL,hand_key=NULL,grip_key=NULL,equipped=0 WHERE entry_id=?").run(command.recipientActorId,entry.entry_id);
         } else {
           ...
           if(destination) db.prepare("UPDATE rpg_inventory_entries_v25 SET quantity=quantity+? WHERE entry_id=?").run(command.quantity,destination.entry_id);
            else db.prepare("INSERT INTO rpg_inventory_entries_v25(entry_id,campaign_id,actor_id,item_pack_id,item_pack_version,item_kind,item_definition_id,entry_mode,quantity,instance_key,slot_key,hand_key,grip_key,equipped,created_at) VALUES(?,?,?,?,?,'item',?,'stackable',?,NULL,NULL,NULL,NULL,0,?)").run(deps.ids.nextId(),command.campaignId,command.recipientActorId,command.item.packId,command.item.packVersion,command.item.definitionId,command.quantity,now);
         }
       }
  ...
  return {getActorInventorySnapshot:snapshot,mutateInventoryForActor(principal,campaignId,actorId,input){
    const command:InventoryCommand=input.kind==="equip"?{type:"equip_inventory_item",campaignId,actorId,entryId:input.entryId,slot:input.slot as any,...(input.hand===undefined?{}:{hand:input.hand}),...(input.grip===undefined?{}:{grip:input.grip}),expectedRevision:input.expectedRevision,idempotencyKey:input.idempotencyKey}
