...
import {
  ...
  travelCommandSchema,
  establishCampCommandSchema,
  utcIsoTimestampSchema,
  worldCommandSchema,
  type TravelCommand,
  type ActorTravelCommandRequest,
  type ActorCampCommandRequest,
...
export type ActorTravelResult=Omit<ActorTravelCommandResponse,"receipt">&{campaignId:string;sessionId:string;
   receipt:{commandId:string;idempotencyKey:string;revisionBefore:number;revisionAfter:number;occurredAt:string}};
export type ActorCampResult={campaignId:string;sessionId:string;locationId:string;elapsedMinutes:number;
  receipt:{commandId:string;idempotencyKey:string;revisionBefore:number;revisionAfter:number;occurredAt:string}};
...
export interface WorldWriteRepository {
  ...
  establishCamp(principalId:string,actorId:string,input:ActorCampCommandRequest):ActorCampResult;
...
export function createWorldWriteRepository(
  ...
  function setActorLocation(principalId: string, sessionId: string, raw: unknown): MutationReceipt {
    context.guard(); return db.transaction(() => { const mutation = begin(principalId, sessionId, setActorLocationCommandSchema.parse(raw), "set_actor_location", () => requireGm(principalId, (raw as any).campaignId)); if (mutation.replay) return JSON.parse(mutation.replay.canonical_result_json); const command = mutation.command;
      if (!db.prepare("SELECT 1 FROM campaign_actors WHERE campaign_id=? AND id=?").get(command.campaignId, command.actorId) || !db.prepare("SELECT 1 FROM campaign_locations_v28 WHERE campaign_id=? AND location_id=?").get(command.campaignId, command.locationId)) throw new WorldUnavailableError("actor or location is unavailable");
      const existing = db.prepare("SELECT location_id FROM campaign_actor_locations_v28 WHERE campaign_id=? AND actor_id=? AND session_id=?").get(command.campaignId, command.actorId, sessionId) as { location_id: string } | undefined;
      const result = receipt(mutation, command.idempotencyKey); record(command, sessionId, "set_actor_location", command.actorId, mutation, result, "actor_location_set", { actorId: command.actorId, locationId: command.locationId });
      if (existing) db.prepare("UPDATE campaign_actor_locations_v28 SET location_id=?,state_revision=state_revision+1,updated_at=? WHERE campaign_id=? AND actor_id=? AND session_id=?").run(command.locationId, mutation.at, command.campaignId, command.actorId, sessionId); else db.prepare("INSERT INTO campaign_actor_locations_v28 VALUES(?,?,?,?,0,?)").run(command.campaignId, command.actorId, command.locationId, sessionId, mutation.at);
      // Any authoritative relocation interrupts a camp established at the old location.
      if (existing?.location_id !== command.locationId) db.prepare("UPDATE world_expeditions_v60 SET camp_location_id=NULL,camp_command_id=NULL WHERE campaign_id=? AND session_id=? AND camp_location_id IS NOT NULL AND camp_location_id<>?").run(command.campaignId, sessionId, command.locationId);
      return result;
    }).immediate();
  }
  ...
  function establishCamp(principalId:string,actorIdInput:string,input:ActorCampCommandRequest):ActorCampResult{
    context.guard();const actorId=resourceIdSchema.parse(actorIdInput),intent=actorCampCommandRequestSchema.parse(input);
    return db.transaction(()=>{
      requireGm(principalId,intent.campaignId);
      const sessions=db.prepare(`SELECT attached.session_id FROM campaign_sessions attached JOIN sessions session ON session.id=attached.session_id
        WHERE attached.campaign_id=? AND session.state='active' AND session.stopped_at IS NULL ORDER BY attached.attached_at,attached.session_id`).all(intent.campaignId)as Array<{session_id:string}>;
      if(sessions.length!==1)throw new WorldConflictError("campaign world session is ambiguous");const sessionId=sessions[0]!.session_id;
      const mutation=begin(principalId,sessionId,establishCampCommandSchema.parse({type:"establish_camp",campaignId:intent.campaignId,actorId,
        expectedRevision:intent.expectedRevision,idempotencyKey:intent.idempotencyKey}),"establish_camp",()=>requireGm(principalId,intent.campaignId));
      ...
      const location=db.prepare("SELECT location_id FROM campaign_actor_locations_v28 WHERE campaign_id=? AND session_id=? AND actor_id=?")
        .get(intent.campaignId,sessionId,actorId)as {location_id:string}|undefined;
      if(!location)throw new WorldUnavailableError("actor location is unavailable");
      const expedition=db.prepare("SELECT elapsed_minutes FROM world_expeditions_v60 WHERE campaign_id=? AND session_id=?")
        .get(intent.campaignId,sessionId)as {elapsed_minutes:number}|undefined;
      const result={campaignId:intent.campaignId,sessionId,locationId:location.location_id,elapsedMinutes:expedition?.elapsed_minutes??0,
        receipt:{commandId:mutation.commandId,idempotencyKey:intent.idempotencyKey,revisionBefore:mutation.before,revisionAfter:mutation.after,occurredAt:mutation.at}};
      record(mutation.command,sessionId,"establish_camp",actorId,mutation,result,"camp_established",{locationId:location.location_id});
      if(expedition)db.prepare("UPDATE world_expeditions_v60 SET camp_location_id=?,camp_command_id=? WHERE campaign_id=? AND session_id=?")
        .run(location.location_id,mutation.commandId,intent.campaignId,sessionId);
      else db.prepare("INSERT INTO world_expeditions_v60 VALUES(?,?,?,?,?)").run(intent.campaignId,sessionId,0,location.location_id,mutation.commandId);
      return result;
    }).immediate();
  }
  ...
  function resolveFactionReaction(principalId:string,factionIdInput:string,input:FactionReactionCommandHttpRequest):FactionReactionResult{
    ...
    return db.transaction(()=>{
      ...
      if(!faction)throw new WorldUnavailableError("faction is unavailable");const campaignId=faction.campaign_id;
      if(!gm(principalId,campaignId))throw new WorldAuthorizationError("GM authority is required");
      const observation=db.prepare(`SELECT observation_id FROM agent_observations
        WHERE campaign_id=? AND agent_kind='faction' AND agent_id=? AND source_command_id=?
        ORDER BY hop_count ASC, created_at DESC, observation_id ASC LIMIT 1`)
        .get(campaignId,factionId,intent.sourceCommandId) as {observation_id:string}|undefined;
      if(!observation)throw new WorldUnavailableError("faction has no knowledge to react to");
      const reputationIntent:FactionReputationCommandHttpRequest={subjectActorId:intent.subjectActorId,
        delta:intent.delta,reason:intent.reason,expectedRevision:intent.expectedRevision,idempotencyKey:intent.idempotencyKey};
      const result=changeFactionReputation(principalId,factionId,reputationIntent);
