...
export function executeCombatCompositionPlan(
  ...
  for (const mirror of plan.healthMirrors) {
    ...
    if (!root) db.prepare("INSERT INTO rpg_m15_mutation_revisions_v25(campaign_id,actor_id,revision,updated_at) VALUES(?,?,0,?)")
      .run(plan.campaignId, mirror.actorId, plan.occurredAt);
    ...
    db.prepare(`INSERT INTO rpg_m15_commands_v25(command_id,campaign_id,actor_id,command_family,command_type,
      ...
      VALUES(?,?,?,'resource','encounter_health_mirror',?,?,?,?,?,?)`).run(mirror.commandId, plan.campaignId,
        ...
        mirror.revisionAfter, plan.occurredAt);
    const keys = '["resource:health"]', keysDigest = createHash("sha256").update(keys).digest("hex");
    db.prepare("INSERT INTO rpg_m15_receipts_v25 VALUES(?,?,?,?,?,?,?,?,?)").run(plan.campaignId, mirror.actorId,
      mirror.commandId, mirror.revisionAfter, mirror.resultJson, mirror.resultDigest, keys, keysDigest,
      plan.occurredAt);
    db.prepare("INSERT INTO rpg_m15_receipt_changed_keys_v25 VALUES(?,?,?,?,?)").run(plan.campaignId,
      mirror.actorId, mirror.commandId, "resource:health", mirror.revisionAfter);
  }
  failpoint?.("health");
  ...
  for (const advance of plan.effectAdvances) {
    ...
    if (!root) db.prepare("INSERT INTO rpg_m16_mutation_revisions_v26 VALUES(?,?,0,?)")
      .run(plan.campaignId, advance.actorId, plan.occurredAt);
    db.prepare("INSERT INTO rpg_m16_commands_v26 VALUES(?,?,?,?,?,?,?,?,?,?,?)")
      .run(plan.campaignId, advance.actorId, advance.commandId, "effect", "advance_effect_duration",
        ...
        advance.revisionAfter, plan.occurredAt);
    db.prepare("INSERT INTO rpg_m16_receipts_v26 VALUES(?,?,?,?,?,?,?)")
      .run(plan.campaignId, advance.actorId, advance.commandId, advance.revisionAfter,
        advance.resultJson, advance.resultDigest, plan.occurredAt);
    ...
    db.prepare("INSERT INTO rpg_m16_events_v26 VALUES(?,?,?,?,?,?,?,?)")
      .run(advance.eventId, plan.campaignId, advance.actorId, advance.commandId, advance.revisionAfter,
        "effect_duration_advanced", advance.resultJson, plan.occurredAt);
