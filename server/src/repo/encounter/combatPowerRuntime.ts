...
export type CombatPowerLegalAction={legalActionId:string;encounterId:string;campaignId:string;actingCombatantId:string;sourceActorId:string;
  targetCombatantId:string;targetActorId:string|null;powerRef:PowerReference;definition:Definition;cost:{kind:"slot";id:string}|{kind:"ability-use";id:string}|null};
...
export function buildCombatPowerLegalActions(db:DatabaseDriver.Database,principal:string,encounterId:string):CombatPowerLegalAction[]{
  ...
  for(const power of powers){let definition:Definition;try{definition=power.kind==="spell"?spellCatalogDefinitionSchema.parse(JSON.parse(power.definition_json)):abilityCatalogDefinitionSchema.parse(JSON.parse(power.definition_json));}catch{continue;}
    const kind=supported(definition);if(!kind)continue;const ref=definition.reference as PowerReference;
    ...
    if(ref.kind==="ability"&&mechanics.uses>0){if(usedCount(db,encounter.campaign_id,acting.actor_id!,ref,mechanics.recovery)>=mechanics.uses)continue;cost={kind:"ability-use",id:ref.definitionId};}
...
function adjustment(db:DatabaseDriver.Database,action:CombatPowerLegalAction,target:Row,type:string,at:string):"none"|"resistance"|"vulnerability"|"immunity"{
  const values:string[]=[];if(target.actor_id){for(const row of db.prepare(`SELECT modifier_kind kind FROM rpg_active_effects_v26 effect JOIN rpg_effect_modifiers_v26 modifier USING(effect_id)
    WHERE effect.campaign_id=? AND effect.actor_id=? AND effect.status='active' AND (effect.duration_kind<>'rounds' OR effect.remaining_rounds>0) AND (effect.duration_kind<>'until_timestamp' OR effect.expires_at>?) AND modifier.applies_to_id IN (?, 'all')`).all(action.campaignId,target.actor_id,at,type)as any[])values.push(row.kind);}
  else{const raw=db.prepare(`SELECT definition.definition_json FROM encounter_enemy_provenance_v31 provenance JOIN rpg_catalog_definitions definition ON definition.pack_id=provenance.pack_id AND definition.pack_version=provenance.pack_version AND definition.kind=provenance.kind AND definition.definition_id=provenance.definition_id WHERE provenance.combatant_id=?`).get(target.combatant_id)as any;
    if(raw){const enemy=JSON.parse(raw.definition_json);if(enemy.mechanics.immunities.includes(type))values.push("immunity");if(enemy.mechanics.resistances.includes(type))values.push("resistance");if(enemy.mechanics.vulnerabilities.includes(type))values.push("vulnerability");}}
  if(values.includes("immunity"))return"immunity";const resistant=values.includes("resistance"),vulnerable=values.includes("vulnerability");return resistant===vulnerable?"none":resistant?"resistance":"vulnerability";
}
...
export function executeCombatPower(db:DatabaseDriver.Database,deps:EncounterDependencies,principal:string,input:CombatPowerRequest,failpoint?:(boundary:CombatPowerBoundary)=>void):CombatPowerResult{return db.transaction(()=>{
  ...
  const at=utcIsoTimestampSchema.parse(deps.clock.now().toISOString()),kind=supported(action.definition)!;let hp=target.hit_points;const outcomes:any[]=[];
  for(const effect of action.definition.mechanics.effects){if(effect.type==="damage"){const roll=evaluateDiceExpression(`${effect.dice.count}d${effect.dice.sides}${effect.dice.modifier===0?"":effect.dice.modifier>0?`+${effect.dice.modifier}`:effect.dice.modifier}`,deps.rng),requested=Math.max(0,roll.total),adjust=adjustment(db,action,target,effect.damageType,at),adjusted=adjust==="immunity"?0:adjust==="resistance"?Math.floor(requested/2):adjust==="vulnerability"?requested*2:requested,before=hp,absorbed=absorbDamage(db,action.encounterId,target.combatant_id,adjusted,at);hp=Math.max(0,hp-absorbed.hitPointDamage);outcomes.push({kind:"damage",damageType:effect.damageType,roll,requested,adjustment:adjust,applied:before-hp,temporaryHitPointsAbsorbed:adjusted-absorbed.hitPointDamage,temporaryHitPointsAfter:absorbed.temporaryHitPointsAfter,before,after:hp});}
    else if(effect.type==="healing"){const roll=evaluateDiceExpression(`${effect.dice.count}d${effect.dice.sides}${effect.dice.modifier?`+${effect.dice.modifier}`:""}`,deps.rng),requested=Math.max(0,roll.total),before=hp;hp=Math.min(target.maximum_hit_points,hp+requested);outcomes.push({kind:"healing",roll,requested,applied:hp-before,before,after:hp});}}
  ...
  let replaced=false;if(kind==="effect"){const effect=action.definition.mechanics.effects[0]as any,targetActor=target.actor_id!,targetRevisionNow=revision(db,"m16",action.campaignId,targetActor),targetAfter=targetRevisionNow+1,effectCommandId=nextId(deps),effectId=nextId(deps),concentration=action.definition.reference.kind==="spell"&&(action.definition as any).mechanics.concentration;
    ...
    const condition=effect.type==="condition",duration=condition?{kind:"rounds",remaining:effect.durationRounds}:effect.duration==="turn"||effect.duration==="round"?{kind:"rounds",remaining:1}:{kind:"until_removed"},modifier={kind:"flat",appliesToId:condition?`condition:${effect.condition}`:effect.statistic,amount:condition?1:effect.amount};
  ...
  db.prepare("INSERT INTO rpg_power_uses_v26 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(powerUseId,action.campaignId,action.sourceActorId,commandId,sourceAfter,action.powerRef.packId,action.powerRef.packVersion,action.powerRef.kind,action.powerRef.definitionId,"slot",action.powerRef.kind==="spell"?(action.definition as any).mechanics.level||1:1,target.actor_id,envelope,at);if(action.cost)db.prepare("INSERT INTO rpg_power_use_costs_v26 VALUES(?,?,?,?,1)").run(powerUseId,0,action.cost.kind==="slot"?"slot":"charge",action.cost.id);db.prepare("INSERT INTO rpg_m16_events_v26 VALUES(?,?,?,?,?,?,?,?)").run(nextId(deps),action.campaignId,action.sourceActorId,commandId,sourceAfter,"power_used",resultJson,at);db.prepare("UPDATE rpg_m16_mutation_revisions_v26 SET revision=?,updated_at=? WHERE campaign_id=? AND actor_id=? AND revision=?").run(sourceAfter,at,action.campaignId,action.sourceActorId,sourcePowerBefore);
  failpoint?.("combatant");
