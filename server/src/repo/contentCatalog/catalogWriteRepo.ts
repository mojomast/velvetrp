...
export function createCatalogWriteRepository(db: DatabaseDriver.Database, deps: CatalogWriteDependencies) {
  ...
  const configureCampaignCatalog = (actor: string, campaignIdValue: string, raw: unknown): CampaignCatalogConfigurationResult => {
    ...
    return db.transaction(() => {
      ...
      if (existingSelection) { db.prepare("DELETE FROM rpg_campaign_catalog_definitions_v25 WHERE campaign_id=?").run(campaignId); db.prepare("DELETE FROM campaign_catalog_current_selections WHERE campaign_id=?").run(campaignId); db.prepare("DELETE FROM campaign_content_packs WHERE campaign_id=?").run(campaignId); db.prepare("DELETE FROM campaign_rules_profiles WHERE campaign_id=?").run(campaignId); }
