...
function safeProblemInstance(requestTarget: string): string {
  ...
  if (/^\/api\/rpg\/v1\/npcs\/[^/]+\/relationship-commands$/.test(path)) {
    return "/api/rpg/v1/npcs/:npcId/relationship-commands";
  }
  ...
  if (/^\/api\/rpg\/v1\/combats\/[^/]+\/log$/.test(path)) {
    return "/api/rpg/v1/combats/:combatId/log";
  }
  if (/^\/api\/rpg\/v1\/combats\/[^/]+\/action-commands$/.test(path)) {
    return "/api/rpg/v1/combats/:combatId/action-commands";
  }
  ...
  if (/^\/actors\/[^/]+\/inventory$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/actors/:actorId/inventory`;
  }
  if (/^\/actors\/[^/]+\/inventory-commands$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/actors/:actorId/inventory-commands`;
  }
  if (/^\/actors\/[^/]+\/rest-commands$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/actors/:actorId/rest-commands`;
  }
  if (/^\/actors\/[^/]+\/wallet$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/actors/:actorId/wallet`;
  }
  if (/^\/actors\/[^/]+\/economy-commands$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/actors/:actorId/economy-commands`;
  }
  if (/^\/shops\/[^/]+$/.test(suffix)) {
    return `${campaignPrefix}:campaignId/shops/:shopId`;
  }
