...
function openDb(dir: string, dependencies: RepositoryDependencies): DatabaseDriver.Database {
...
function getDb(): DatabaseDriver.Database {
  ...
  const db = openDb(dir, systemRuntime);
  connection = { dir, db };
  return db;
}
...
function migrateLegacyIfPresent(db: DatabaseDriver.Database, dir: string, dependencies: RepositoryDependencies): void {
...
function validateOriginalStarterCharacterContent(
  ...
  if (!referencesMatch || input.controllerPrincipalId !== "local-owner") {
    throw new CampaignCharacterCreationConflictError();
  }
...
function listRecentCampaignDiceEventsSync(
  db: DatabaseDriver.Database,
  actorPrincipalId: string,
  campaignId: string,
  timelineId: string,
...
function getCommandReceiptSync(
  db: DatabaseDriver.Database,
  actorPrincipalId: string,
  campaignId: string,
  commandId: string,
...
export function createRepository(options: CreateRepositoryOptions = {}): Repository {
  ...
  const db = openDb(path.resolve(options.dataDir ?? resolveDataDir()), dependencies);
  let closed = false;
  ...
  return {
    ...
    executeRollActorDice: (actorPrincipalId, envelope) => {
      ...
      return executeRollActorDiceSync(db, dependencies, actorPrincipalId, envelope);
    },
    executeRollActorDiceForVisibleCharacter: (actorPrincipalId, envelope, binding) => {
      ...
      return executeRollActorDiceForVisibleCharacterSync(
        db, dependencies, actorPrincipalId, envelope, binding,
      );
    },
...
export async function listCharacters(): Promise<Character[]> {
  const rows = getDb().prepare("SELECT * FROM characters ORDER BY created_at ASC, rowid ASC").all() as CharacterRow[];
  return rows.map(toCharacter);
}
...
export async function createCharacter(input: CreateCharacterInput): Promise<Character> {
  return createCharacterSync(getDb(), systemRuntime, input);
}
...
export async function recordUsageEvent(sessionId: string, kind: string, usage: TokenUsage): Promise<void> {
  getDb().prepare(`INSERT INTO usage_events (id, session_id, source_message_id, kind, prompt_tokens, completion_tokens, total_tokens, usage_source, usage_model, created_at)
    VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?)`)
    .run(randomUUID(), sessionId, clampText(kind, 80), usage.promptTokens, usage.completionTokens, usage.totalTokens, usage.source, usage.model, now());
}
...
export async function getHarnessSettings(): Promise<HarnessSettings> {
  return readHarness(getDb());
}

function updateHarnessSettingsSync(
  db: DatabaseDriver.Database,
  clock: Clock,
...
export async function updateHarnessSettings(patch: UpdateHarnessInput): Promise<HarnessSettings> {
  return updateHarnessSettingsSync(getDb(), systemRuntime.clock, patch);
}
