...
function validateContent(content:GeneratedCampaignContentProvider,sections:string[],dependencies:Map<string,"public"|"gm">,catalogReferences:Set<string>):GeneratedCampaignContentProvider{
  ...
  for(const value of content.encounters){check(value,[...value.participantNpcKeys,...value.monsterConceptKeys,...(value.locationKey?[value.locationKey]:[])]);for(const reference of value.enemyReferences)if(!catalogReferences.has(referenceIdentity(reference)))throw new Error("encounter enemy reference is not pinned to this campaign");}
