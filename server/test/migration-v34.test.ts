...
describe("schema v34 story domain", () => {
  it("has migration parity and preserves legacy roots as empty graphs", () => {
    ...
    const migrated = new DatabaseDriver(file(), { readonly: true }); expect(migrated.prepare("SELECT value FROM meta WHERE key='schemaVersion'").get()).toEqual({ value: "34" }); migrated.close();
