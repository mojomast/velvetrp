...
function fixture() {
  ...
  // before their turn, which replaces the action set with a lone death save and removes dash.
  const repo = createRepository({ clock: { now: () => new Date(AT) }, ids: { nextId: () => `dash-${++sequence}` }, rng: { integer: (minimum: number) => minimum } });
