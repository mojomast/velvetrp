...
import {
  createRepositoryForCorruptionTests,
  type CreateRepositoryOptions,
} from "../src/repo/campaignRepositoryOrchestration.js";
import type { Repository } from "../src/repo/campaign/index.js";
...
/** Opens a repository only after a test has deliberately malformed its persisted fixture. */
export function createCorruptionTestRepository(options: CreateRepositoryOptions = {}): Repository {
  return createRepositoryForCorruptionTests(options);
}

/** Registers the hostile function only for deliberate campaign-deletion corruption fixtures. */
...
/** Removes a campaign inside a deliberate corruption fixture, then restores the current trigger DDL. */
    ...
    if (req.method === "POST" && req.url === "/v1/chat/completions") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          // Ignore malformed bodies in the fake provider.
        }
        const respond = () => {
