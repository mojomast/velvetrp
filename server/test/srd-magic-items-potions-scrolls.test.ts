...
import { magicPotionsScrolls } from "../src/content/srdStarter/magicItems/potions-scrolls.js";
...
const items = magicPotionsScrolls(refs);
const parsed = items.map((item) => itemCatalogDefinitionSchema.parse(item));

describe("SRD 5.1 potions and scrolls", () => {
  ...
  it("carries magic-item mechanics and category tags on every definition", () => {
    for (const definition of parsed) {
      ...
      expect(definition.mechanics.category).toBe("consumable");
