...
describe("schema", () => {
  it("records the fresh v47r1 schema, current inventory, and foreign-key integrity", async () => {
    ...
    expect(raw.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND (name IN (
    ...
    ) OR name GLOB '*v43*' OR name GLOB '*v45*' OR name GLOB '*v46*' OR name GLOB '*v47*') ORDER BY name`).all()).toEqual(expect.arrayContaining([
      { name: "campaign_companions_v45" },
      { name: "exact_candidate_executions_v47" },
      { name: "exact_candidate_execution_layout_attestation_v47" },
      ...
      { name: "companion_audit_events_v45" },
      { name: "companion_commands_v45" },
      { name: "companion_decision_receipts_v45" },
      { name: "companion_decisions_v45" },
      { name: "companion_grant_command_families_v45" },
      { name: "companion_grant_revocations_v45" },
      { name: "companion_grants_v45" },
      { name: "companion_layout_attestation_v45" },
      { name: "companion_presence_links_v45" },
      { name: "companion_proposals_v45" },
      { name: "companion_receipts_v45" },
