...
The separate `srd-5.1:starter@1.1.0+2b1f05336aac` publication preserves that 1.0.3 publication unchanged and adds only the SRD Fighter table and class features needed for the exact Human/Acolyte/Fighter path: level 1 maximum d10 hit points, level 2 fixed hit-point increase (6 before Constitution modifier), proficiency bonus +2, Second Wind, and Action Surge (SRD 5.1 PDF pp. 25-26). That immutable publication remains historical; the current starter carries forward its bounded Fighter progression. Fighter level 3 Martial Archetype (PDF p. 25) is explicitly unsupported.

The current `srd-5.1:starter@1.2.0+7f94bb928392` publication preserves prior definitions and adds SRD ancestry, equipment, cantrip, level-one spell, and level-two/three class-level records. Ancestry traits, expanded spell effects, subclass-dependent progression, and Action Surge remain metadata-only. Cleric Bless, Cure Wounds, Healing Word, and Magic Missile, Fighter Second Wind, Barbarian Rage resistance state, and Paladin Lay on Hands have bounded server execution; ranged/thrown attacks, full-cover geometry, and opportunity reactions require the authoritative tactical state described in the coverage inventory.
