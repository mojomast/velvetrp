...
The server-internal campaign context basket is not exposed through `/api/harness`, a prompt-template layer, or an RPG wire contract. At the current **v48r1**, **102-operation** boundary, canonical populated v46/v47->v48 upgrades are supported and v45 and earlier are unsupported. M5.1 presence remains the cast authority. Wave A includes fixed-local-owner companion administration transport, the consumable HTTP/client lane, v46 exact-candidate issuance/lifecycle, v47 execution links, and v48 provider/adventure exact travel binding, recovery, receipt-only HTTP/client display, and safe narration; none changes harness configuration or catalog attestation. Provider-committed travel E2E is delivered using fixture-only prerequisites/inspection. Live candidate generation/selection HTTP/client APIs remain absent, and existing manual travel remains separate. The deterministic fixture additions are harness-only, and the coherent custom immutable published-pack setup does not alter production catalog rules. The basket's exact conflict precedence is: safety/control; human canon; committed mechanics; the exact final user declaration; visible world/cast/quests and legal actions; authorized private target facts; approved memory/lore; recap/summary; generated suggestions. When supplied to legacy player/NPC generation, it replaces the ordinary triggered-lore, retrieved-memory, and shared-context prompt blocks so those retrieval sources are not duplicated. Generations without campaign context keep the existing 20-layer behavior.
