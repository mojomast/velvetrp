...
The current trusted-local RPG boundary has 145 counted explicit operations plus separately classified feature discovery, excluding implicit HEAD; this supersedes the historical count above. The prior preparation/recovery checkpoint had 135 counted operations. The ten director additions cover campaign control read/mode changes, explicit scene-evidence binding, room history, private preparation readiness, public run/private proposal reads, and beat/decision/resume commands. See the [API inventory](api.md#campaign-director) for exact paths and provider-free versus provider-capable behavior.
