...
The v46 migration adds immutable exact-candidate batches, candidates, explicit same-scope supersessions, optional expiration observations, canonical frame/digest storage, and layout attestation. It performs no backfill. Decision and execution-receipt links are intentionally absent until a later additive schema. The normal application has no candidate generator or world execution adapter, so these tables remain empty outside direct repository use and tests; there is no provider advertisement, route, client, or E2E for this persistence foundation.
