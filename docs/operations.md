...
| `FEATURE_SYSTEM_ONE` | server-runtime | Disabled | Exact `true` advertises optional System One (Jev) decision lanes; rollout control only. No lane is wired to a request path yet, every lane falls back deterministically, and none mutates campaign state. |
