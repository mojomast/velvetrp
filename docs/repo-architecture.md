...
| M5.3 repository consumables | Shared encounter contracts freeze the exact item, inventory entry, target, effect plan, canonical request digest, expected M15 revisions, and result evidence. `encounter/useConsumableRuntime.ts` owns server-derived legal actions and one immediate-transaction execution for exact pinned category-`consumable` quantity one. The runtime applies effects in catalog order for damage, healing, and health/guard/focus resources; instant modifiers remain fail-closed because authoritative semantics are absent. `encounterReadRepo.ts` exposes separate repository-only legal-action and immutable result readers. No route/client or live combat-union wiring is delivered. |
