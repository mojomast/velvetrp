...
| `campaignRepo.ts` | The composed synchronous `Repository`/`RepositoryUnitOfWork` implementation and the current campaign/RPG persistence owner: campaigns, timelines, room attachment, content packs/configuration, campaign characters/sheets/actors, command/event/receipt audit behavior, authorized projections, and starter operations. It opens factory-owned connections, injects clock/ID/RNG ports, composes helpers from legacy domain repositories, and delegates the dice-specific surface to `diceRepo.ts`. Private access, membership, session-attachment, room-linking, character-read, actor-resource-read, and event-projection adapters own their focused reads while this facade retains event reconstruction. The filename is historical; its composition role is broader than campaign CRUD. |
| `campaign/campaignAccessRepo.ts` | Private DB-scoped adapter for membership-rooted campaign list/get projections. It preserves authorization masking and sole-owner integrity validation in factory and unit-of-work reads. |
| `campaign/campaignMembershipReadRepo.ts` | Private owner-authorized campaign membership list/get projection. It owns canonical membership parsing, target-parent evidence, binary ordering, and non-disclosing denial. |
| `campaign/campaignSessionAttachmentReadRepo.ts` | Private owner-only metadata reader for campaign session attachments. It preserves opaque legacy session IDs and excludes session presentation/private data. |
| `campaign/campaignRoomLinkingSnapshotRepo.ts` | Private bounded, path-bound room-linking snapshot reader. It validates authority and session summaries without selecting private session or character payload. |
| `campaign/campaignCharacterReadRepo.ts` | Private role-sensitive campaign-character aggregate reader. It owns CASE-gated private fields and same-campaign ancestry validation; roster, workspace, and progression snapshots remain separate actor projections. |
| `campaign/campaignActorResourceRepo.ts` | Private campaign actor-resource read adapter composed by `campaignRepo.ts` for factory and unit-of-work paths. It owns membership-rooted, role-safe current resource projections and strict persisted-row validation. It is distinct from the M1.5 `actorResourceRepo.ts` command facade. |
...
- `campaignRepo.ts -> campaign/campaignActorResourceRepo.ts` composes a private read adapter for factory and unit-of-work resource projections. It does not own M1.5 resource commands or receipts.
- `campaignRepo.ts -> campaign/campaignAccessRepo.ts`, `campaignMembershipReadRepo.ts`, `campaignSessionAttachmentReadRepo.ts`, `campaignRoomLinkingSnapshotRepo.ts`, and `campaignCharacterReadRepo.ts` composes private read adapters in both factory and unit-of-work paths. They preserve existing authorization, integrity, and projection boundaries without widening public APIs.
