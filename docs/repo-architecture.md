...
| M5.4 exact candidates | `candidateRepo/` and `db/migrations/v46_exact_candidates.ts` own batches, candidates, supersession, expiration-observation, attestation, and authoritative travel candidate generation. `world/actorTravelPolicy.ts` and `world/actorTravelTransaction.ts` are the shared policy and connection-scoped mutation core used by the explicit session adapter and both public actor/legacy travel wrappers. Candidate selection/execution and receipt links are not delivered; neither are provider/tool advertising, candidate routes/client, or candidate E2E. Existing manual travel remains public and is not candidate-backed. |
