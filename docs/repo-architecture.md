...
Current persistence is schema v24r1. V23 introduced M1.4 progression; v24 repaired its provenance/integrity graph with fixed canonical DDL digest `e056d9df1ec9f9c00cc1aba740f2acc91b40cc7b03a5716cb75e79ec8df6bec8`.
...
| `campaignAdministrationRepo.ts` | Connection-scoped schema-v15 campaign administration facade composed by `campaignRepo.ts`. It owns lifecycle/settings projections, revision-checked idempotent administration commands, audited membership and room changes (including deterministic compatibility audits for legacy signatures), checkpoint state snapshots and inherited-prefix forks, recaps, bounded package validation/apply, role-safe receipt/log reads, and export manifests. Its parallel immutable command/event/receipt family intentionally does not widen or rebuild the closed gameplay audit tables. |
...
- `campaignRepo.ts -> campaignAdministrationRepo.ts` composes the administration facade with the same private connection and runtime ports. The administration module imports only the composition dependency type in return, avoiding a runtime cycle.
...
M1.1-M1.4 remain repository/shared-contract capabilities. The trusted-local boundary now has 21 operations: the historical 14 plus server-only builder create/read/update, progression read/preview, and administration GET/PATCH. These adapters share the parent feature plugin's one lazy repository and close hook, use relative child paths, and fix `local-owner`; no client/UI is implied. The mechanics starter remains separate from the original metadata-only starter.
