...
- **Calibration.** Fitting the recorded confidence with a Platt map reduced expected calibration
  ...
  0.0569 to 0.0031 (router, acted), 0.1350 to 0.0036 (rerank, decisive) and 0.0508 to 0.0022
...
- **Adoption.** Room routing is the first promoted lane and may act when explicitly configured
  (`active` lane mode plus a passing record); adventure-selection followed with the first active
  check path on 2026-09-18 (three live canary turns committed DC-15 checks as `origin=lane`
  executions). The other lanes carry frozen promotion records and stay record-only
  (adventure-selection remains shadow for every non-check family).
...
| --- | --- | --- | --- | --- |
| L1 `director-selection` | Pick the next Director beat from server-issued candidates | Promoted / evidence-only; wired shadow (record-only); no active path | At the calibrated 0.60 threshold, a `--repeat 10` run acted on 41/60 samples with 100% acceptable and 100% exact; calibrated ECE 0.0069 (held-out 0.0054), decision stability 100% (0 of 6 scenarios conflicted). Measurement sensitivities are recorded: `--repeat 8` fell to 29 acted (below the gate) and a uid-decorrelated probe moved the threshold to 0.30 with calibrated ECE 0.1344. The benchmark also merges 9 human-confirmed harvested live cases (confirmed against the review sheet on 2026-09-17); that run passes with 94 acted/100% (calibrated ECE 0.0068), but the record still uses the frozen-only measurement. No acted errors, so the calibration tail is untested. Gated decision 303 ms vs 1473 ms before. | [Director calibration](system-one-director-calibration.md), [Director benchmark](system-one-director-benchmark.md) |
| L2 `adventure-selection` | Pick one exact advertised adventure-turn candidate | Promoted at the sweep threshold / active for SRD check commits and rest proposals; shadow for every other family | 348 calls over 30 frozen cases plus 86 harvested cases (1 human-confirmed + 85 agent-reviewed synthetic; production-shaped, no `uid`). The gate uses frozen + human-confirmed only: 54 named a candidate with all 54 picks acceptable, 100% accuracy at the sweep-recommended 0.40, Brier 0.0002, ECE 0.0110; decision stability 100% (0 of 116 cases conflicted). The 85 agent-reviewed cases are scored separately (90.6% on their asserted subset, none acted) and never gate the record. The human drop-my-longsword case is a stable deferred miss; server default stays 0.75. The first active path is live: four lane-origin DC-15 checks committed across a canary and a synthetic batch, and rest picks append a normal proposal that still requires confirmation. | [Adventure benchmark](system-one-adventure-benchmark.md), [harvest loop](system-one-harvest-loop.md) |
