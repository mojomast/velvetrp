...
| L2 `adventure-selection` | Pick one exact advertised adventure-turn candidate | Promoted at the sweep threshold / active for SRD check commits; shadow for every other family | 348 calls over 30 frozen cases plus 86 harvested cases (1 human-confirmed + 85 agent-reviewed synthetic; production-shaped, no `uid`). The gate uses frozen + human-confirmed only: 54 named a candidate with all 54 picks acceptable, 100% accuracy at the sweep-recommended 0.40, Brier 0.0002, ECE 0.0110; decision stability 100% (0 of 116 cases conflicted). The 85 agent-reviewed cases are scored separately (90.6% on their asserted subset, none acted) and never gate the record. The human drop-my-longsword case is a stable deferred miss; server default stays 0.75. The first active path is live: four lane-origin DC-15 checks committed across a canary and a synthetic batch. | [Adventure benchmark](system-one-adventure-benchmark.md), [harvest loop](system-one-harvest-loop.md) |
