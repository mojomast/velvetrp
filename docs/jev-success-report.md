...
| L2 `adventure-selection` | Pick one exact advertised adventure-turn candidate | Promoted at the sweep threshold / evidence-only; wired shadow (record-only) | 129 calls over 30 frozen cases plus 13 harvested cases (1 human-confirmed, 12 agent-reviewed synthetic; production-shaped, no `uid`), 57 named a candidate with all 57 picks acceptable; at the server default 0.75 only 6 acted; sweep-recommended 0.40: 57 acted, 100% accuracy, Brier 0.0001, ECE 0.0105. Decision stability 100% (0 of 43 cases conflicted). Threshold selected on the same corpus that scores it; all 13 harvested cases defer (the human drop-my-longsword case is a stable deferred miss); server default stays 0.75. | [Adventure benchmark](system-one-adventure-benchmark.md), [harvest loop](system-one-harvest-loop.md) |
