...
| L2 `adventure-selection` | Pick one exact advertised adventure-turn candidate | Promoted at the sweep threshold / active for SRD check commits; shadow for every other family | 258 calls over 30 frozen cases plus 56 harvested cases (1 human-confirmed + 55 agent-reviewed synthetic; production-shaped, no `uid`). The gate uses frozen + human-confirmed only: 54 named a candidate with all 54 picks acceptable, 100% accuracy at the sweep-recommended 0.40, Brier 0.0002, ECE 0.0110; decision stability 100% (0 of 86 cases conflicted). The 55 agent-reviewed cases are scored separately (92.7% on their asserted subset, none acted) and never gate the record. The human drop-my-longsword case is a stable deferred miss; server default stays 0.75. The first active path is live: three canary turns committed DC-15 checks as `origin=lane` executions on 2026-09-18. | [Adventure benchmark](system-one-adventure-benchmark.md), [harvest loop](system-one-harvest-loop.md) |
