...
- **Next work:** before allocating v46, freeze M5.4's stable turn-scoped `connectionId`, single-actor party, no-confirmation policy, behavior for more than 32 routes, expiry/supersession, and receipt mapping. M5.2 may still add a minimal companion management UI, and M5.5 persistence/command/client integration remains Planned. The protected catalog remains unchanged.
...
- **Pre-v46 semantic freeze:** decide stable turn-scoped `connectionId`, require a single-actor party, freeze no-confirmation policy, define behavior when authoritative visibility yields more than 32 routes, define expiry and supersession, and map candidate execution to the travel receipt. Allocate v46 only after these semantics are frozen.
...
- **Docs/commits:** checkpoint delivered as `a2b479c feat(contracts): freeze exact candidate protocol`. Persistence/repository and the first travel adapter remain Planned and gated by the pre-v46 semantic freeze and connection-scoped world-domain design.
