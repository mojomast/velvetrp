...
**Authority and status.** This is the actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers. [ROADMAP.md](ROADMAP.md) owns scope and milestone status; this document is subordinate to it and supplies implementation detail. Runtime contracts and current code remain authoritative for shipped behavior. Current persistence is v45r1, the supported startup window is canonical populated v43/v44->v45, v42 and earlier are unsupported, and the current trusted-local RPG surface remains 97 explicit operations.
...
- **Schema allocation:** M5.1 delivered **v43r1**, the M5.2 foundation delivered **v44r1**, and durable companion principals delivered **v45r1**. M5.3 Slice 0, its consumable contract prerequisite, and the M5.4/M5.5 contract/vector checkpoints are no-schema. A later persistence slice receives a number from its schema steward only at implementation start, and each allocation moves the rolling support window.
...
After H0.4, these tracks may proceed in parallel:
  M5.1 NPC presence -> M5.2 v45 repository administration -> HTTP/client administration
  M5.3 combat-health Slice 0 -> consumable contract prerequisite -> atomic runtime Slice 1
  M5.4 exact-candidate v1 checkpoint Complete -> gated persistence/travel adapter
...
- **Wave A progress:** M5.2 repository-only administration, M5.3 Slice 0 and consumable contract prerequisite, and the pure M5.4/M5.5 protocol checkpoints are now delivered. These are bounded slices, not full milestone completion.
- **Next exact operation:** implement the atomic connection-scoped consumable planner/executor. Add M5.2 HTTP administration contracts, routes, and client only after preserving the repository boundary. M5.4 persistence/travel-adapter work and M5.5 persistence/command/client integration remain Planned and gated by their domain designs. The protected catalog remains unchanged.
...
**Status:** In Progress/partial; repository-only administration delivered, HTTP/client administration and later companion lifecycle work Planned. **Schema:** v45r1 delivered.

- **Outcome/dependencies:** the delivered repository boundary extends existing NPC identity and v43 session presence rather than replacing either model. Owner/GM can create a companion from a persisted present NPC in an attached running session and create or revoke bounded grants. The next M5.2 slice may expose administration through HTTP/client only after this repository boundary.
...
- **Likely next files/services:** shared HTTP administration contracts, routes, and client over the delivered repository boundary; later campaign context and `confirmationPolicy.ts` integration. Reuse NPC presence rather than inventing a second location model.
...
- **Security/migration:** v45 replaces v44 sidecars and preserves all rows while durable principals decouple retained history from membership lifecycle. Supported startup coverage is canonical populated v43/v44->v45; v42 and earlier are unsupported. Revocation wins immediately; expiry uses server time; no self-grant, cross-campaign grant, caller principal, or grant exercise before L5.
...
- **Docs/commits:** latest delivery is `42d9934 feat(repo): add v45 companion principals` and `839f70f feat(repo): administer companion grants`; routes/UI, dismissal, proposal/decision administration, grant exercise, context integration, and final status remain later slices.
...
**Status:** In Progress/partial; no-schema Slice 0 and consumable contract prerequisite delivered, atomic runtime Slice 1 Planned. **Schema:** no M5.3 schema allocated.
...
- **Delivered consumable prerequisite:** the shared contract freezes exact item, inventory entry, target, effect plan, canonical request digest, expected acting/target M15 revisions, and result evidence. Policy requires an exact pinned category-`consumable` item and quantity one; admits damage, healing, health/guard/focus resource effects, and instant modifiers; explicitly excludes spell-slot effects because no spell level is available, plus conditions, non-instant effects, and nonconsumables. It is not wired to a repository executor or live route, and current route unions remain unchanged.
- **Planned Slice 1:** implement the atomic connection-scoped planner/executor, with server policy spending the acting turn's action regardless of catalog `actionCost`.
...
- **Docs/commits:** Slice 0 delivered as `68b3c57 feat(repo): compose authoritative combat health`; consumable contracts delivered as `ff2610c feat(contracts): define combat consumable protocol`. Runtime item execution, routes/UI, and final status remain later work.
...
**Status:** pure exact-candidate v1 contract/vector checkpoint Complete; persistence and adapters Planned. **Schema:** none allocated.

- **Outcome/dependencies:** the pure contract work is complete. Persistence and adapter work remain separately gated by their exact domain mechanics and lifecycle design. Every eventual agent mutation references an exact, reviewable server-generated candidate or quote instead of provider-authored command arguments.
...
- **Validation:** the pure contract/vector checkpoint is represented by its focused protocol tests. Later persistence/adapters add candidate repository, policy, orchestrator, command-adapter, restart/reconciliation, projection/security, migration/support-window, and deterministic pause/resume/execute E2E coverage as their boundaries are implemented.
- **Docs/commits:** checkpoint delivered as `a2b479c feat(contracts): freeze exact candidate protocol`. Persistence/repository and the first travel adapter remain Planned and gated by lifecycle/session and connection-scoped world-domain design.
