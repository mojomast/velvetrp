...
**Authority and status.** This is the actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers. [ROADMAP.md](ROADMAP.md) owns scope and milestone status; this document is subordinate to it and supplies implementation detail. Runtime contracts and current code remain authoritative for shipped behavior. Current persistence is v44r1, the supported startup window is canonical populated v42/v43->v44, and the current trusted-local RPG surface remains 97 explicit operations.
...
After H0.4, these tracks may proceed in parallel:
  M5.1 NPC presence -> M5.2 v44 foundation -> repository administration core
  M5.3 combat-health Slice 0 -> consumable prerequisites/policy -> Slice 1
...
- **Wave A progress:** M5.2 contracts and v44 foundation, M5.3 Slice 0, and the pure M5.4/M5.5 protocol checkpoints are now delivered. These are bounded slices, not full milestone completion.
- **Next exact operation:** continue the M5.2 repository administration core and M5.3 consumable prerequisites/policy. M5.4 persistence/travel-adapter work and M5.5 persistence/command/client integration remain gated by their domain designs. The protected catalog remains unchanged.
...
**Status:** In Progress/partial; contracts and empty persistence foundation delivered, repository administration core Planned. **Schema:** v44r1 delivered.

- **Outcome/dependencies:** the delivered contracts and v44 sidecars extend existing NPC identity and v43 session presence rather than replacing either model. The next slice implements the repository model for owner/GM administration, explicit revocable control, companion-owned proposals and decisions, receipts, and role-safe projections using the existing principal model.
...
- **Delivered foundation:** immutable revisioned companion commands/receipts and empty companion, presence-link, proposal, decision, decision-receipt, grant, grant-command-family, revocation, audit, and layout-attestation sidecars. No repository commands/projections, routes, UI, context integration, or grant exercise are delivered by this foundation.
- **Likely next files/services:** companion repository administration and projections; later campaign access/context reads, `confirmationPolicy.ts`, routes, and UI. Reuse NPC presence rather than inventing a second location model.
...
- **Security/migration:** v44 additively delivered the companion/proposal/decision/grant/revocation/audit foundation with deterministic empty backfill and moved supported startup coverage to canonical populated v42/v43->v44. Revocation must win immediately; expiry uses server time; no self-grant, cross-campaign grant, caller principal, or grant exercise before L5.
...
- **Docs/commits:** delivered `9efd346 feat(contracts): define companion core` and `7c41064 feat(repo): add v44 companion foundation`; repository administration, grants/confirmation, context/routes/UI, and final status remain later slices.
...
**Status:** In Progress/partial; no-schema Slice 0 delivered, consumable Slice 1 Planned/blocked. **Schema:** no M5.3 schema allocated.
...
- **Planned Slice 1:** one fixed combat `use consumable` command, with server policy spending the acting turn's action regardless of catalog `actionCost`, remains blocked until exact inventory/action transaction contracts are settled. The intended bounded subset still requires one exact pinned category-`consumable` item with quantity >= 1 and only damage, healing, resource, or instant-modifier effects.
...
- **Docs/commits:** Slice 0 delivered as `68b3c57 feat(repo): compose authoritative combat health`; consumable prerequisites/policy, item path, routes/UI, and final status remain later work.
