...
**Authority and status.** This is the actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers. [ROADMAP.md](ROADMAP.md) owns scope and milestone status; this document is subordinate to it and supplies implementation detail. Runtime contracts and current code remain authoritative for shipped behavior. Current persistence is v47r1, the supported startup window is canonical populated v45/v46->v47, v44 and earlier are unsupported, and the current trusted-local RPG surface has 102 explicit operations.
...
- **Next work:** provider/adventure integration for the delivered exact-candidate repository execution boundary. Provider/tool advertising, candidate routes, client, and E2E remain later slices. M5.2 may still add a minimal companion management UI, and M5.5 persistence/command/client integration remains Planned. The protected catalog remains unchanged.
...
**Status:** In Progress/partial; v46 issuance/lifecycle persistence and v47 repository-only exact travel execution are delivered, while provider/routes/client/E2E exposure remains undelivered. **Schema:** v47r1.

- **Outcome/dependencies:** the pure contract work, v46 issuance/lifecycle persistence, shared travel core, candidate generator, and v47 repository-only atomic selection/execution with receipt linking are complete. Provider/adventure integration is next. Every eventual agent mutation references an exact, reviewable server-generated candidate or quote instead of provider-authored command arguments.
...
- **Delivered files/services:** exact-candidate contracts; `candidateRepo/`; v46 issuance/lifecycle and v47 execution-link migrations; repository composition/types; migration support fixtures/tests; shared world travel policy and connection-scoped transaction core; explicit session adapter; public actor wrapper; authoritative candidate generation; and atomic repository selection/execution with immutable receipt links. **Later files/services:** separately approved adventure/provider, route, client, and E2E integration.
...
- **Security/migration:** v46 owns immutable issuance and lifecycle history. v47 adds one immutable attested execution-link table with empty backfill; repository verification reconstructs original batches and all canonical candidate, selection, linked-envelope, result, command, receipt, event, destination, and party evidence. Supported populated upgrades are v45/v46->v47; v44 is rejected before future-artifact cleanup. Provider/adventure integration is next; provider/tool, routes, client, and E2E remain absent.
...
- **Validation:** the final repository-only v47 gate passed 41 exact-candidate contract tests and 126 server migration/support/candidate/world/repository tests, plus contract and server typechecks. Active discovery includes five v47 migration tests and three support-window tests. No E2E was run because provider and candidate HTTP/client boundaries were not added.
- **Docs/commits:** protocol and v46 persistence delivery commits are `e0ef5f8`, `a690e3a`, and `71393a9`; prerequisite delivery commits are `bd921b8 refactor(repo): unify authoritative travel execution` and `c05a289 feat(repo): generate exact travel candidates`. The v47 repository selection/execution and immutable receipt-link implementation commit is pending. Provider/tool advertising, candidate HTTP/client surface, and E2E remain absent.
