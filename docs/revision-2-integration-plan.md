...
**Authority and status.** This is the actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers. [ROADMAP.md](ROADMAP.md) owns scope and milestone status; this document is subordinate to it and supplies implementation detail. Runtime contracts and current code remain authoritative for shipped behavior.

**Research baseline.** Repository research was performed against `e0025a41f1393a61dcab9d24e08d1804d374169b` and the preserved dirty worktree on 2026-08-11. Current persistence is v42r1 and M4 is complete. Existing dirty migration archive/support-window fixtures, documentation, and planning-board work are evidence to finish rather than reset. File paths called **observed** exist at that baseline; paths called **likely/new** are proposed ownership, not claims that a file exists. The saved board and repository can evolve independently, so each milestone starts with a focused contract/code recheck and records any discovery without silently changing the approved product decision.

## Approved scope and interpretation

### Build now

1. Establish a tested rolling current-minus-two migration window, currently canonical populated v40 and v41 upgrades to v42, while retaining older migration tests only as an explicit archive.
...
- **Schema allocation:** only M5.1 has the immediate provisional allocation **v43r1**. Every later persistence milestone says **`vNext` allocated by the schema steward at milestone start**. Numbers are not reserved in advance; each allocated version moves the rolling support window.
...
After H0.4, these tracks may proceed in parallel:
  M5.1 NPC presence -> M5.2 companion aggregate/local-owner administration
  M5.3 atomic combat
  M5.4 candidate protocol contract/persistence
  M5.5 dice threat-model checkpoint -> commit-reveal dice
...
**Status:** In Progress; not delivered. **Schema:** provisional v43r1, the only preallocated future version.

- **Outcome/dependencies:** after H0.4, model which campaign NPCs are present in a campaign session, optionally at one visible campaign location, so context/UI no longer infer presence from the full roster.
- **Start recheck evidence:** a read-only contract/code/schema/fixture recheck confirms current v42r1, no v43 or NPC-presence artifact drift, intact canonical v40/v41 support fixtures, provisional v43r1 availability, and no blockers. This is start evidence only, not shipped or feature-validation evidence.
- **Scope/decisions:** additive presence aggregate keyed by campaign/session/NPC with optional location, status, revision, entered/left timestamps, and GM command provenance. Presence is session-scoped; a location, when set, must belong to the campaign and obey visibility. GM/owner place, move, and remove; reads return role-safe present cast. A stopped-session projection is structurally historical at-stop and every presence write rejects after stop. Detach blocks only while a running attached session has live-present rows; stopped-session detach remains allowed and historical presence survives detachment.
- **Likely files/services:** new `packages/contracts/src/npc-presence*.ts`; `server/src/repo/world/`, `worldRepo.ts`, campaign context/play reads, `npcRoutes.ts`/`worldRoutes.ts`, RPG route composition; cast/context drawer/play client components; v43 migration and focused contract/repo/route/client tests.
...
- **Security/migration:** additive v43 tables plus deterministic backfill of **no presence rows** (unknown is not absent). Fresh, populated canonical v41/v42→v43, malformed FK, rollback, attestation, and parity tests move the support window to v41/v42. No historical layout edits.
- **Non-goals:** no companion model, autonomous NPC speech, inferred co-location, pathfinding, schedule, remote identity, or multiple active encounters.
...
- **Validation:** new contract tests; focused world/context/repo/route/client tests; migration v43 plus support-window tests; owning workspace typechecks; deterministic E2E because browser/HTTP/persistence boundaries change.
- **Docs/commit boundaries:** contracts+design; migration/repository; routes; context/client; docs/status. Code/test/contracts commits require the owning typecheck (root for cross-workspace scope); the docs/status commit uses targeted consistency/link checks plus `git diff --check`. Update API, repo architecture, operations, feature docs, ROADMAP/devplan/handoff before completion.
...
**Status:** Planned. **Schema:** `vNext` allocated by the schema steward at milestone start.

- **Outcome/dependencies:** after M5.1, persist companion identity/presence and the repository model for explicit revocable control. This milestone may implement the aggregate, closed grant records, repository-derived authority, owner/GM administration, receipts, role-safe projections, and repository tests using the existing principal model.
- **Scope/decisions:** companion aggregate links a fictional persona/NPC-like actor to campaign/session presence; closed grants record grantee, allowed command families, actor/resource scope, maximum spend/use, start/expiry, revocation, and confirmation policy. Creation, scope expansion, transfer, and consequential use require confirmation. Under the current trusted-local HTTP model, all HTTP/client requests still execute as fixed `local-owner`: an owner/GM can administer companion/grant state where authorized, but a distinct non-owner grantee cannot exercise companion controls through HTTP/client pre-auth. Context and repository authorization derive authority from persisted relationships and the server-supplied principal; no impersonation field or header workaround is permitted.
- **Likely files/services:** new companion contracts/repository/routes/UI; campaign access/context reads; `confirmationPolicy.ts`, adventure-turn confirmation persistence, and migration. Reuse NPC presence rather than inventing a second location model.
...
- **Security/migration:** additive companion/grant/audit sidecars; deterministic empty backfill; current-minus-two fixtures shift with the allocated version. Revocation wins immediately; expiry uses server time; no self-grant, cross-campaign grant, or caller principal.
...
- **Docs/commits:** contracts+threat notes; migration/core; grants/confirmation; context/routes/UI; docs/status. Apply the global scope-based code versus docs-only validation policy to each commit.
...
**Status:** Planned. **Schema:** `vNext` allocated by the schema steward at milestone start only if persistence changes are required.
...
- **Scope/decisions:** move cross-domain execution to a composition-owned unit of work; command planners produce immutable plans from authoritative state, and connection-scoped executors consume them inside the encounter transaction. Preserve one active encounter per campaign session.
...
- **Security/migration:** expose safe roll/cost/delta labels; hide enemy private stats, inventory details, tactics, internal IDs, principal, and raw command plan. Prefer no schema change; if normalized composed sidecars are needed, use additive migration and shift support fixtures.
- **Non-goals:** no simultaneous encounters, full grids, boss phases, arbitrary effects, client-calculated damage, bilateral trade, or autonomous combat.
- **Acceptance:** injected failure at every write boundary leaves all state/revisions/logs unchanged; exact retry returns the same receipt with no RNG/clock/ID/resource reuse; stale inventory or resource rejects before any write; reconnect reconciles one result.
...
- **Docs/commits:** plan/read model; composition transaction; power path; item path; routes/UI; docs/status. Apply the global scope-based code versus docs-only validation policy to each commit.
...
- **Outcome/dependencies:** contract and persistence work may start independently after H0.4. Every agent mutation eventually references an exact, reviewable server-generated candidate or quote instead of provider-authored command arguments; each command adapter waits only for its exact domain mechanic.
- **Scope/decisions:** generic persisted candidate envelope includes candidate kind/version, campaign/session/actor scope, safe label/summary, canonical action digest, server-owned private parameters, expected revisions, policy result/reason, confirmation requirement, quote/cost where applicable, expiry, supersession, and execution receipt link. Provider-safe lists contain only exact allowable selections; provider responses select candidate IDs plus bounded visible choices. Provider work is outside transactions.
- **Likely files/services:** contracts for provider-safe candidate projections and internal envelopes; `server/src/agent/toolRegistry.ts`, `adventureOrchestrator.ts`, `confirmationPolicy.ts`; adventure-turn repo/migration; command adapters for each family; confirmation/receipt client UI.
- **Authority/transaction/projection:** candidate generation is an authoritative read/quote service; execution rechecks digest, actor/campaign/session, policy, grant, expiry, revision, and current legal state inside the domain command transaction. Candidate creation/execution linking is idempotent. Public/provider projections omit private args, principals, opaque domain IDs not required for selection, hidden state, policy internals, and provider metadata.
- **Security/migration:** additive candidate, quote, policy-decision, supersession, and receipt-link sidecars; empty backfill; current-minus-two fixtures shift. Candidates are single-purpose, bounded, expiring, non-transferable, and non-authorizing by themselves.
...
- **Validation:** contract, candidate repository, policy, orchestrator, command-adapter, restart/reconciliation, projection/security, migration/support-window tests; server/root typechecks; deterministic E2E for pause/resume/execute.
- **Docs/commits:** protocol contract/threat model; migration/repository; policy; first adapter; orchestration/UI; docs/status. Apply the global scope-based code versus docs-only validation policy to each commit.
- **Rollback/recovery:** turn off candidate execution while retaining immutable records; expire/supersede pending candidates; never reinterpret old candidate payloads under a new version.

### M5.5 — Commit-reveal verifiable dice

**Status:** Planned. **Schema:** `vNext` allocated by the schema steward at milestone start.

- **Outcome/dependencies:** after an explicit threat-model checkpoint following H0.4, this track may proceed independently so clients can verify that a committed roll seed was not changed after declaration while deterministic tests retain injected randomness. It is not a blanket prerequisite for travel, rest, or non-random mechanics, and does not depend on the generic candidate protocol except where a proof must bind a candidate.
- **Scope/decisions:** define protocol/version, server commitment, client entropy or nonce when available, reveal, transcript digest, bounded dice expression, result linkage, timeout/cancellation, crash recovery, and proof verification. Commit precedes outcome-relevant declaration finalization; reveal sidecars bind exact command/candidate/roll.
...
- **Security/migration:** additive commitment/reveal/proof sidecars linked to existing immutable rolls; historical rolls remain valid but “unverifiable legacy,” never synthetic-backfilled proofs. Threat model covers withholding, replay, nonce reuse, ordering, concurrent rolls, and malicious clients/providers. Shift support fixtures.
...
- **Acceptance:** independent verifier reproduces result/digest; commitment mismatch/reuse/cross-command proof rejects; withheld reveal has explicit terminal recovery; retries never reroll; deterministic injected fixtures are stable.
- **Validation:** cryptographic vector/contract/repository/command/route/client verifier tests, migration/support-window tests, security review, workspace typechecks, deterministic E2E for commit→reveal→verify and restart/abandon.
- **Docs/commits:** threat model/protocol; migration; command integration; verifier/UI; docs/status. Apply the global scope-based code versus docs-only validation policy to each commit.
