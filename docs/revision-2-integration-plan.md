...
After H0.4, these tracks may proceed in parallel:
  ...
  M5.3 combat-health Slice 0 -> consumable contract -> repository atomic runtime -> route/client wiring
...
- **Wave A progress:** M5.2 repository-only administration, M5.3 Slice 0 plus contract and repository-only atomic consumable execution, and the pure M5.4/M5.5 protocol checkpoints are now delivered. These are bounded slices, not full milestone completion.
- **Next parallel tasks:** deliver M5.2 HTTP administration contracts, routes, and client adding two operations; wire M5.3 routes/client for only the supported consumable runtime subset. Do not wire instant modifiers. M5.4 persistence/travel-adapter work and M5.5 persistence/command/client integration remain Planned and gated by their domain designs. The protected catalog remains unchanged.
...
**Status:** In Progress/partial; no-schema Slice 0 plus consumable contract and repository atomic runtime delivered, route/client integration Planned. **Schema:** no M5.3 schema allocated.
...
- **Docs/commits:** Slice 0 delivered as `68b3c57 feat(repo): compose authoritative combat health`; consumable contracts as `ff2610c feat(contracts): define combat consumable protocol`; repository runtime and readers as `e27402d feat(repo): settle combat consumables atomically`. `cd885dc test(repo): align v45 corruption assertions` corrects fixture expectations from marker 44 to current marker 45. Routes/UI and final status remain later work.
