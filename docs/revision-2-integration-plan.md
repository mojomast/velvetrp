...
- **Schema allocation:** M5.1 delivered **v43r1**. No schema beyond v43 is allocated; M5.2 uses **`vNext`**, and every later persistence milestone receives **`vNext` from its schema steward at milestone start**. Numbers are not reserved in advance; each allocated version moves the rolling support window.
...
- **Next exact operation:** launch parallel, disjoint, read-only rechecks for M5.2 companion core, M5.3 atomic combat, M5.4 candidate protocol, and the M5.5 dice threat-model checkpoint. No schema beyond v43 is allocated until each schema steward starts; M5.2 uses `vNext`.
