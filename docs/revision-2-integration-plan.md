...
**Authority and status.** This is the preserved actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers at that checkpoint. It is now a historical approved plan, not the owner of current status or next work. [ROADMAP.md](ROADMAP.md) owns current scope/milestone status, [api.md](api.md) owns HTTP behavior, [operations.md](operations.md) owns current migration support, and [repo-architecture.md](repo-architecture.md) owns persistence structure. Status, schema, count, and “next” language below applies to this saved revision unless explicitly labeled current.
...
- **Migration window at this saved revision:** the board proposed canonical populated `N-2` and `N-1` support. That historical plan is retired; current development databases are disposable and [operations.md](operations.md) is authoritative.
- **Schema allocation:** M5.1 delivered **v43r1**, the M5.2 foundation delivered **v44r1**, durable companion principals delivered **v45r1**, M5.4 issuance/lifecycle delivered **v46r1**, repository exact execution delivered **v47r1**, and provider/adventure exact travel binding, recovery, accounting, receipt projection, and safe narration delivered **v48r1**. M5.3 Slice 0, its consumable contract and repository runtime, and the M5.5 contract/vector checkpoint are no-schema.
...
After H0.4, these tracks may proceed in parallel:
  ...
  M5.4 exact-candidate v1 checkpoint Complete -> v46 persistence -> travel core + candidate generator -> v47 selection/execution -> provider/adventure integration
...
- **Wave A progress:** M5.2 repository administration plus HTTP/client transport, M5.3 Slice 0 plus the separate consumable HTTP/client/UI flow, the M5.4 protocol and v46 persistence checkpoints plus authoritative travel-core and candidate-generation prerequisites, and the pure M5.5 protocol checkpoint are delivered. These are bounded slices, not full milestone completion.
- **Next work:** live candidate generation/selection HTTP and client selection remain later slices. Provider/tool travel execution, receipt-only HTTP/client display, and one provider-committed candidate E2E are delivered. M5.2 may still add a minimal companion management UI, and M5.5 persistence/command/client integration remains Planned. The protected catalog remains unchanged.
...
- **Security/migration:** at the historical v45 delivery checkpoint, v45 replaced v44 sidecars and preserved all rows while durable principals decoupled retained history from membership lifecycle; canonical populated v43/v44->v45 was that checkpoint's support window. Current support is governed by [operations.md](operations.md); the v47 material below is historical. Revocation wins immediately; expiry uses server time; no self-grant, cross-campaign grant, caller principal, or grant exercise before L5.
...
**Status:** In Progress/partial; v46 issuance/lifecycle, v47 exact execution, v48 provider/adventure exact travel, receipt-only HTTP/client display, and provider-committed travel E2E are delivered. Live candidate generation/selection HTTP and client APIs remain undelivered. **Schema:** v48r1.

- **Outcome/dependencies:** the pure contract work, v46 issuance/lifecycle persistence, shared travel core, candidate generator, v47 atomic execution, and v48 provider/adventure exact travel binding/recovery/narration are complete. Receipt-only product integration reuses the existing command-receipt GET and CampaignPlay receipt links, and deterministic E2E proves one provider-committed travel plus reload. Live candidate generation/selection HTTP and client selection remain absent. Every eventual agent mutation references an exact, reviewable server-generated candidate or quote instead of provider-authored command arguments.
- **Delivered checkpoint:** exact-candidate protocol v1 freezes the envelope, canonical frames/digests, provider-safe projection, exhaustive kind policy, lifecycle/decision/supersession/expiry/quote/receipt bindings, and fail-closed execution gates. Travel projections add bounded public origin/destination labels per opaque candidate while selection remains candidate ID/kind/version only; legacy persisted v1 candidates project a deterministic route-option label. Its first and only current v1 kind is `actor.travel`; adding a family requires an explicit protocol decision rather than silently widening v1.
...
- **Delivered files/services:** exact-candidate contracts; `candidateRepo/`; v46 issuance/lifecycle, v47 execution-link, and v48 provider-binding migrations; repository composition/types; migration support fixtures/tests; shared world travel policy and connection-scoped transaction core; explicit session adapter; public actor wrapper; authoritative candidate generation; atomic repository selection/execution with immutable receipt links; adventure/provider selection, recovery, accounting, safe narration; existing receipt GET/client display; and provider-committed travel E2E. **Later files/services:** separately approved live candidate generation/selection routes and client selection.
...
- **Security/migration:** v46 owns immutable issuance/lifecycle history; v47 owns immutable execution links; v48 immutably binds provider-safe projection and strict tool schema, settled provider call/tool/round/selection, execution, and accounting. Recovery converges after settled-response or post-execution crashes, and narration/receipt display expose only safe travel facts. Supported populated upgrades are v46/v47->v48; v45 and earlier reject before mutation. Live candidate generation/selection HTTP/client APIs remain absent; provider-committed travel E2E is delivered.
...
- **Validation:** focused contract, migration/support, candidate, orchestrator, route/repository, client receipt, and typecheck groups pass. The deterministic provider-committed travel E2E passes through production candidate/provider/execution/receipt paths; `git diff --check` passes.
- **Docs/commits:** protocol and v46 persistence delivery commits are `e0ef5f8`, `a690e3a`, and `71393a9`; prerequisite delivery commits are `bd921b8 refactor(repo): unify authoritative travel execution` and `c05a289 feat(repo): generate exact travel candidates`; v47 delivery commits are `638204e`, `c2b2309`, `01a8e2f`, `f486159`, and `09aef20`; v48/provider delivery is `18abe26`; receipt-only product delivery is `5031489`; deterministic E2E is `29d4de5`. Live candidate generation/selection HTTP and client selection remain absent.
