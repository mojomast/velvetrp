...
**Authority and status.** This is the actionable execution design for planning-board revision 2, saved 2026-08-11T00:47:24.296Z with `ready: true` and no blockers. [ROADMAP.md](ROADMAP.md) owns scope and milestone status; this document is subordinate to it and supplies implementation detail. Runtime contracts and current code remain authoritative for shipped behavior. Current persistence is v43r1, the supported startup window is canonical populated v41/v42->v43, and the current trusted-local RPG surface has 97 explicit operations.
...
- **Schema allocation:** M5.1 delivered **v43r1**. The completed M5.2-M5.5 read-only rechecks allocated no schema, so no schema beyond v43 is currently allocated. The M5.2 schema steward may allocate **v44 only when implementation starts**; M5.3 Slices 0/1 allocate no schema only while limited to the fixed combat subset below, and M5.4/M5.5 pure contract/vector work allocates no additional schema. A later persistence milestone receives a number from its schema steward only at its own implementation start, and each allocation moves the rolling support window.
...
After H0.4, these tracks may proceed in parallel:
  ...
  M5.5 dice threat-model checkpoint Complete -> commit-reveal dice Planned
...
- **Recheck completion:** the parallel, disjoint M5.2-M5.5 rechecks completed read-only. They froze readiness decisions but delivered no milestone implementation and allocated no schema; only the M5.5 threat-model checkpoint changed status to Complete.
- **Next exact operation:** preserve parallelism by starting M5.2 schema/contracts with its schema steward allocating v44, the conditional no-schema M5.3 Slices 0/1 subset in disjoint files, M5.4 pure contract work, and M5.5 exact versioned binary framing, domain separation, HKDF/HMAC inputs, rejection/timeout protocol, and vectors only, with no additional schema. Assign one integration owner to shared files. The protected catalog remains unchanged.
...
**Status:** Planned; read-only contract recheck Complete, contract frozen/ready. **Schema:** none allocated; the schema steward may allocate v44 only when implementation starts.

- **Outcome/dependencies:** after M5.1, implement a companion sidecar over existing NPC identity and v43 session presence rather than a replacement actor or presence model. Persist the repository model for explicit revocable control, companion-owned proposals and decisions, owner/GM administration, receipts, role-safe projections, and repository tests using the existing principal model.
...
- **Security/migration:** when implementation starts, the schema steward may allocate v44 for additive companion/proposal/decision/grant/audit sidecars with deterministic empty backfill; current-minus-two fixtures then shift. No schema was allocated by the recheck. Revocation wins immediately; expiry uses server time; no self-grant, cross-campaign grant, caller principal, or grant exercise before L5.
...
**Status:** Planned; read-only recheck Complete, Slices 0/1 ready only for the fixed subset below. **Schema:** none allocated; no schema is required only while those slices remain within that subset.
...
- **Scope/decisions:** Slice 0 freezes current representable semantics and Slice 1 moves cross-domain execution to a composition-owned unit of work. While an encounter is active, encounter combatant HP is authoritative and actor health is mirrored atomically in the same transaction. Round-based durations decrement only at round wrap, the only currently representable anchor. Concentration persists until explicit replacement or removal; this milestone adds no damage-break rule. The item subset is one fixed combat `use consumable` command: server policy spends the acting turn's action regardless of catalog `actionCost`, consumes one unit from an exact pinned item whose quantity is >= 1, and permits only category `consumable` items whose effects are damage, healing, resource, or modifiers with `instant` duration. Command planners produce immutable plans from authoritative state, and connection-scoped executors consume them inside the encounter transaction. Preserve one active encounter per campaign session.
...
**Status:** Planned; read-only recheck Complete, ready for pure contract/protocol freeze. **Schema:** none allocated; the freeze is no-schema.
...
- **Scope/decisions:** freeze a generic exact candidate envelope before persistence: candidate kind/version, campaign/session/actor scope, safe label/summary, canonical action digest, server-owned private parameters, expected revisions, exhaustive per-kind policy result/reason, confirmation requirement, quote/cost where applicable, expiry, supersession, and execution receipt link. Provider-safe projections contain only exact allowable selections; provider responses select candidate IDs plus bounded visible choices. Supersession, expiry, quote, decision, and receipt links are explicit and versioned. Provider work is outside transactions.
...
- **Security/migration:** the contract/protocol freeze allocates no schema. Eventual persistence must use additive candidate, quote, policy-decision, supersession, expiry, and receipt-link sidecars with empty backfill. Candidates are single-purpose, bounded, expiring, non-transferable, and non-authorizing by themselves.
...
- **Validation:** the immediate pure freeze runs contract/vector and exhaustive-kind/projection tests plus the owning typecheck, with no migration test. Later persistence/adapters add candidate repository, policy, orchestrator, command-adapter, restart/reconciliation, projection/security, migration/support-window, and deterministic pause/resume/execute E2E coverage as their boundaries are implemented.
- **Docs/commits:** pure protocol contract and vectors first, with no schema; persistence/repository allocation occurs only in a later implementation slice. Travel is the first adapter after lifecycle/session semantics freeze. Every adapter requires its connection-scoped domain boundary. Apply the global scope-based code versus docs-only validation policy to each commit.
...
**Status:** threat-model decisions and primitive-selection checkpoint Complete; exact protocol contract and implementation Planned. **Schema:** none allocated; the next pure contract/vector task is no-schema.

- **Outcome/dependencies:** the completed threat-model checkpoint permits only the next pure-contract task before implementation readiness: specify a verifiable commitment/reveal protocol while deterministic tests retain injected randomness. This track is not a blanket prerequisite for travel, rest, or non-random mechanics, and does not depend on the generic candidate protocol except where a proof must bind a candidate.
- **Scope/decisions:** the checkpoint selected platform cryptography with SHA-256, HKDF, HMAC, and rejection sampling behind an injectable secure-byte port, explicit `pending`, `settled`, `abandoned`, and `legacy` lifecycle states, and detectable-not-preventable withholding. The next pure-contract task must define exact versioned binary framing and domain separation; every HKDF/HMAC input; commitment, client entropy/nonce, reveal, transcript, expression, result, and command/candidate/roll bindings; rejection behavior; timeout/cancellation/abandonment behavior; and independent vectors. These details are not yet frozen, and implementation is not ready until they are settled and reviewed.
...
- **Acceptance:** the next contract checkpoint is Complete only when independent vectors reproduce every framed digest/result; domain and cross-version confusion, commitment mismatch/reuse, malformed or cross-command proofs, invalid rejection paths, and timeout-state violations reject exactly; and deterministic injected secure-byte fixtures demonstrate unbiased bounded-dice rejection sampling. Withheld reveal remains detectable with explicit `abandoned` handling, not preventable.
- **Validation:** the next no-schema task runs the exact protocol vectors, framing/domain-separation/HKDF-HMAC input tests, lifecycle/rejection/timeout contract tests, secure-byte injection tests, the owning typecheck, and security review. Repository/command/route/client verifier, migration/support-window, workspace typecheck, and deterministic commit→reveal→verify plus restart/abandon E2E coverage wait for implementation readiness and implementation of those boundaries.
