...
**Status:** Planned. **Schema:** provisional v43r1, the only preallocated future version.
...
- **Authority/transaction/projection:** one world command validates membership, session attachment, NPC/location ancestry, expected revision, and idempotency in an immediate transaction; state/event/receipt commit together. Player projection exposes public NPC identity/display fields and visible location label only—not goals, controller, principal, private location, command IDs, or hidden absence reasons.
