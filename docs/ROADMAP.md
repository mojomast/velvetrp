...
This roadmap is the sole owner of current milestone status and remaining scope. It grew from the completed gap analysis in [the historical RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md). Development persistence uses one current disposable schema with narrowly recognized exact predecessor upgrades described in [Operations](operations.md#data-directory-and-current-schema); validation failure rolls back and all other unknown schemas reject without repair. Repository modules live under `server/src/repo/`, shared runtime contracts live under `packages/contracts/src/`, and the checked API inventory contains 169 counted trusted-local RPG HTTP operations plus feature discovery.
...
The ordered playability program is deterministically complete: preparation readiness, the reviewed human/AI adventure, and measured campaign memory are delivered. The GM Director now exposes exact no-replay dispatch inspection, and the measured `now`/`current` lexical correction improves development and frozen holdout ranking without privacy, packing, hydration, provider-cost, or schema changes. The optional matched live reading-quality comparison remains undispatched; the prior one-call RouteTok capability probe is the only live evidence. Plan 4 living knowledge is implemented as an immutable `agent_observations` ledger with bounded witness/`told`/faction/town-gossip propagation, trust-gated reads, labeled Director narration, recorded-knowledge quest offers, and a provider-free attribution/privacy/negation evaluator; the plan's clue-source bridge and any first-class faction-reaction command type remain deferred pending narrowly scoped schema migrations, and per-agent belief projection is not claimed. Plan 5 living Director is implemented: ordered 1-3 candidate composition with per-candidate receipts and partial-completion blockers, four closed read-only grounding tools with a capped planning loop, receipt-recorded world time and ambient transition beats with an optional narration question, Plan 4 knowledge in public narration, labeled non-authoritative prior-scene continuity with up to four cast dialogue lines, a provider-free Director candidate-choice oracle plus rubric, and a live playtest harness; browser `e2e/tests/campaign-dm-living.spec.ts` proves transition composition, the question-less transition scene, NPC knowledge attribution, and no paid call on reload. A follow-on client playability phase makes the character-sheet actions, vendor sale, bilateral trades, room detach, actor placement and camp, companion administration, and reviewed encounter generation browser-reachable, each covered by the deterministic E2E gate.
...
After H0.4, parallel tracks are:
  M5.1 NPC presence -> M5.2 repository + HTTP/client transport delivered -> optional management UI
  M5.3 combat-health Slice 0 -> consumable runtime + HTTP/client UI -> modifier exclusion decided
  M5.4 exact-candidate v1 checkpoint Complete -> v46 persistence -> travel core + candidate generator
  M5.5 exact protocol checkpoint Complete -> persistence integration gated
...
### Build now — H0 repository health (Wave H green)
...
- **H0.2 Complete:** the original six failures were four finalization call sites in `e2e/tests/app.spec.ts` expecting `200` versus authoritative `201`, one attached unconfigured room expecting legacy chat/back but routing to play under feature-only routing, and one storyline/quest setup expecting creation while story `POST` returned `400` under the strict graph contract. Repairs preserve the public `201` contract with an E2E-only authorized actor resolver, gate play routing on configured status with cancellation/error handling, and use the strict story/quest workflow with idempotent replay. At that checkpoint M2.5 passed and the deterministic suite contained 12 cases. Validation passed: client focused 3 files/113 tests plus client typecheck; server fixture/M1.5 2 files/17 tests plus server typecheck; `typecheck:e2e`; full deterministic E2E 12 passed; `git diff --check`. Commits: `ee7dfba fix(client): preserve authoritative campaign navigation`; `60afa5f test(e2e): align authoritative RPG workflows`.
- **H0.3 Complete:** at H0.3 completion, active documentation described v42r1, completed M4.1-M4.6, 95 trusted-local RPG HTTP operations versus the historical 92-operation M2.11 baseline, and supported canonical populated v40/v41->v42 startup upgrades. Historical schema, operation, and milestone ledgers remain preserved and visibly distinct from current implementation, Planned, Unscheduled, deferred, and excluded status. Commits: `77ed4b0 docs: reconcile current RPG guidance`; `2a50a41 docs: qualify historical RPG baselines`; `8a9a1c5 docs: mark H0.3 complete`; `8061da1 docs: finalize H0.3 handoff`.
- **H0.4 Complete:** root `npm run health` runs exactly `npm run typecheck`, `npm run build`, `npm test`, and `npm run test:e2e` once each in that order. The final recovery run used a unique `/dev/shm` `TMPDIR` and passed all four phases in order. The first attempt timed out externally; the second exposed a global foreign-key preflight regression, which was corrected and scoped before the final wrapper passed. Independent release/security review found no material issues and confirmed deterministic/live credential isolation. Commits: `3e6c1a6 fix(repo): scope migration integrity preflight`; `f18a081 chore: add canonical health gate`; `7286ded docs: document canonical health gate`; `f6cf177 docs: mark H0 health baseline complete`; `31bec78 docs: finalize H0 handoff`. Wave H/H0 is green; M5.1 is Complete/Delivered, and the committed Wave A slices below do not make M5.2-M5.5 fully complete.

### Build next — M5

- **M5.1 Complete/Delivered (v43r1):** persisted session-root presence records `present`/`left` state and an optional campaign location. Running projections expose the live present cast; stopped projections are structurally historical at-stop. Owner/GM commands place, move, or remove NPCs through a session-root revision, with authority, transition, event, and receipt committed atomically. Two trusted-local routes provide role-safe reads and commands; detach blocks only running sessions with live-present rows and preserves stopped/detached history. Agent context and the client drawer use persisted presence, and client mutations reconcile from authoritative reads. The migration deliberately backfills no presence rows and supports canonical populated v41/v42->v43 upgrades.
- **M5.2 In Progress/partial:** repository administration, the fixed-local-owner HTTP lane, the client transport, and the Cast studio companion-administration panel are delivered. Owner/GM can authoritatively GET one management projection and POST the closed create-companion/create-grant/revoke-grant command union; writes atomically commit revision, idempotency, receipt, and audit state and return only a safe receipt. Fresh authorization remains repository-derived from current owner/GM state. Companion creation, exact grant creation, and grant revocation are browser-reachable and covered by deterministic E2E. Delegated grant exercise, dismissal, proposal/decision administration, and a public member HTTP projection remain undelivered, so exposure does not close the milestone.
- **M5.3 In Progress/partial:** Active encounter HP is authoritative and round effects advance only at wrap. The consumable lane remains unchanged. A separate encounter-owned adventure combat-power lane now composes exact single-target catalog damage/healing and one deterministic self/ally persistent effect, including spell-slot or finite-use consumption, enemy-template and actor-effect damage adjustments, concentration replacement, actor-backed HP/resources/effects, combat log/turn advancement, combat receipt, and immutable redacted adventure receipt in one immediate transaction. It does not weaken or reuse the guarded out-of-combat power executor. Area/multi-target, summons, movement, resource grants, arbitrary/instant modifiers, unsupported timing/mixed effects, attack/save resolution absent from current definitions, and enemy automation remain excluded.
- **M5.4 provider/adventure bridge and receipt display delivered (v48r1):** eligible controlled noncombat player turns idempotently generate provider-safe travel candidates before dispatch. Each opaque candidate carries bounded public origin/destination labels for planning, while the strict server-selected tool still accepts only the exact candidate ID/kind/version selection. Successful provider call/tool call/round/selection/execution are immutably bound and recover without duplicate travel. Receipt narration is checked against committed public facts and deterministically replaced when it contradicts or omits the result. The existing command-receipt GET and CampaignPlay receipt card expose only a verified player-safe destination display name, one world revision advance, and commit time. Deterministic E2E covers one provider-committed travel and reload without a second move. No candidate generation/selection route, browser selector, POST owner, or local ambiguity marker was added.
- **M5.5 checkpoint Complete (no schema):** exact versioned framing, platform SHA-256/HKDF/HMAC, rejection sampling, mandatory observed-commitment -> contributed capability state, lifecycle rules, and proof vectors are frozen. Persistence and command/client integration remain Planned and gated by their domain integration design.
- **v49 character rerolls delivered:** server-roll character drafts support explicit full rerolls with immutable allocation history, one draft revision advance, exact idempotent replay without new RNG, and client controls. Standard array, point-buy, and manual allocations are not random pools and do not expose reroll.
- **v50/v52 campaign generation and expansion delivered:** reviewed sparse section candidates cover outline, arcs, locations, factions, NPCs, quests, encounter concepts, clues, story, handouts, and scene prompts. Paid attempts are durably coordinated, exact concurrent requests coalesce, failed attempts require acknowledgement, selective apply resolves accepted-key dependencies, and standard quest/story/world reads reflect applied canon.
- **v51 materialization, placement, and rewards delivered:** finalization settles starter inventory/wallet grants exactly once; owner/GM can bootstrap actor placement; completed combat exposes recipient-safe reward bundles, explicit atomic wallet claim, and exact read-only claim reconciliation without POST replay.
- **v53 campaign material delivery delivered:** generated story graphs carry authoritative provenance; encounter concepts remain inert planning; public handouts and scene prompts require explicit idempotent owner/GM publication and appear in a player-safe published-material projection. GM-only generated material remains non-disclosing.
- **Retained M5.1 exclusions:** no companion, autonomous NPC speech/action, inferred co-location, pathfinding, schedule, remote identity, or multiple active encounters; HTTP remains fixed trusted-local `local-owner`.
...
- **M5.1 validation:** focused contracts; server migration, support-window, NPC-presence repository, detach, route, agent-context, deterministic fixture, and general repository tests; focused client API, drawer, and registry tests; workspace and root typechecks; full deterministic E2E; and `git diff --check` all passed. The final focused nondisclosure regression and server typecheck passed. No totals are asserted by this lifecycle record.
- **Latest Wave A commits:** `409103e feat(api): add companion administration lane`; `77505fb feat(app): expose companion and consumable actions`; `bab31cc fix(repo): share consumable action authority`; `441defc test(e2e): cover companion and consumable flows`; `87d53ab fix(contracts): exclude consumable modifiers`, plus the earlier contract, schema, repository, and rollback commits.
- **Validation:** deterministic Playwright E2E covers companion create/grant/revoke/replay, consumable response-after-commit reload reconciliation against a coherent custom immutable published pack, and provider-committed travel receipt reload. Test totals are intentionally omitted because they change frequently. Deterministic fixture changes remain harness-only; catalog attestation is unchanged.
- **M5.4 commits and validation:** persistence commits are `e0ef5f8`, `a690e3a`, and `71393a9`; delivered travel prerequisites are `bd921b8 refactor(repo): unify authoritative travel execution` and `c05a289 feat(repo): generate exact travel candidates`. Focused contract, server repository/orchestrator/route, client receipt, and typecheck groups pass. Deterministic E2E now proves one provider-committed travel and receipt reload through production paths; live candidate generation/selection HTTP/client APIs remain absent.
- **Next task after drift remediation:** scope the closed declarative rules IR milestone with exact consumers, contract boundaries, current-schema impact, and exclusions before implementation. Live exact-candidate selection, delegated companion grant exercise and dismissal, and M5.5 persistence remain later work until separately promoted.
- **Tactical map integration delivered (v58):** deterministic owner/GM generation, active room/mode and optional encounter binding, authoritative tokens/fog/revisions, role-safe projections, server path/reachability previews, fail-closed combat speed/turn movement, explicit exploration movement, preview-bound idempotent commands, accessible campaign-play Canvas/table UI, and deterministic browser coverage. Timeline checkpoints and campaign transfer exports intentionally exclude live tactical-map state.
- **Current baseline:** one disposable development schema with narrowly recognized exact predecessor upgrades described in [Operations](operations.md#data-directory-and-current-schema); 169 counted operations plus feature discovery. Live exact-candidate generation/selection HTTP and client selection remain absent. Existing manual travel remains separate. The protected catalog is unchanged.
...
- **W0 shipped:** optional System One (Jev) transport lane, strict request/response schemas, a second settings profile (`GET`/`PUT /api/provider/system-one`), an explicit preflight route, the `FEATURE_SYSTEM_ONE` discovery flag, a pure confidence-policy module, and a deterministic fake adapter. The **L5 room-routing lane is wired** behind the flag, setting, and key, with confidence gating to the existing LLM and deterministic paths. It uses a per-participant `noul` battery plus an aggregate best-speaker `choice`, a **shadow mode** for behavior-preserving evaluation, a **lane-scoped budget**, and an **immutable decision-record sidecar**. It is now the **first promoted lane**: `systemOnePromotion.ts` records its measured gate (`isLanePromoted`), so a lane's `active` mode alone is inert until the lane is promoted. Usage records under `room_routing_system_one`. Disabled by default. A Jev-vs-DeepSeek [benchmark](system-one-benchmark.md) is committed alongside the lane.
- **W1 in progress:** the L1 Director selector is wired in **shadow mode** (record-only, behavior unchanged), the evaluation harness now grades predicted probabilities with Brier/ECE and a Director calibration eval runs the selector against the frozen oracle, decision records are readable through a bounded `no-store` API and a shadow-report CLI, and the L3 narration-verification and L7 cost-router **primitives** are implemented as pure, unwired modules. L3 narration-verification and L7 cost-router are now wired in shadow (record-only). The L1 Director battery was **redesigned from measurement**: the single "is this supported?" noul (which conflated legality, relevance, and immediacy and hedged at 0.53–0.73) is replaced by an atomic `progress` noul ("meaningful next step now?") plus a priority `score`, a `hold` noul, and an aggregate `best_candidate` choice. An atomic `legal` question was tried and dropped — the advertised set is server-authorized, so it only added hedging. With zero candidates the lane now forces a hold. Live shadow data then drove a Director **transition-beat fallback** (a new per-candidate `transition` `noul` for pacing beats, composed after grounded `progress` and a confident `hold` and only when every advertised candidate is pacing); the fixture eval still measures **~39 acted, 100% acceptable, and 100% exact** with a **PROMOTE** gate; and the lane's recommended action threshold is **0.60** (the server default remains **0.75** until applied). The [live Director calibration](system-one-director-calibration.md) now runs a **six-state, deterministic, provider-free corpus** (empty world, story graph, revealed story, prepared encounter, active enemy turn, GM-only) and evaluates the promotion gate on the **calibrated** signal. A pure `systemOneCalibration.ts` (Platt map, fit on the development split, scored on the held-out split) fixes the model's systematic under-confidence: at the selected **0.60** threshold a `--repeat 10` run acts on 41/60 samples with **100% acceptable and 100% exact accuracy** (a `--repeat 8` run fell to 29 acted, below the 30-sample gate), and the held-out ECE drops from ~0.21 to ~0.005 (Brier to ~0); a uid-decorrelated probe is recorded as a threshold-fragility finding (selected threshold 0.30, calibrated ECE 0.1344 > 0.10). The gate now reports **PROMOTE**, with an explicit caveat: the frozen corpus currently has no error cases, so calibration cannot be stress-tested, and the promotion is a **candidate** until shadow data with negative examples exists. The fitted map is now **wired into the runtime** as a persisted per-lane `confidenceCalibration` setting (`{ a, b }`, identity by default, clamped on write) applied to the confidence each shadow lane records — it never changes which band a decision lands in. The routing lane has since been **promoted**: its benchmark was extended to score Brier/ECE and the gate, it passed on the calibrated signal (90 acted, 100% exact, ECE 0.003; held-out archive ECE 0.001), and `systemOnePromotion.ts` now carries the frozen record that `isLanePromoted` checks — so a lane acts only when it is both `active` in `laneModes` and promoted. This was verified live (a room turn recorded a `shadow=0, band=act` Jev decision). The global `shadow` boolean is now replaced by a persisted per-lane `laneModes` map (`off`/`shadow`/`active`), so a promoted lane can be `active` while every unpromoted lane keeps recording in `shadow`; legacy persisted `shadow` booleans migrate on read (`true` → every lane `shadow`, `false` → every lane `active`), invalid values are ignored on write, and `enabled` remains the master switch. The Director lane now also carries a promotion record (41 acted on the re-derived `--repeat 10` run, 100% acceptable and 100% exact; `--repeat 8` can fall to 29 acted, below the gate, and a uid-decorrelated probe failed the calibrated gate; the corpus has no acted errors, so it is an evidence-only snapshot) and a **Negative examples** section reports that gap explicitly; a harvested run with 9 human-confirmed live cases passes with 94 acted and 100% acceptable/exact, but the record still uses the frozen-only measurement. No active Director path is wired. The remaining lanes now have their own live evaluations: **L7 cost/quality router** passes its (strict) gate on the calibrated signal (45 acted, 100% handler accuracy, raw ECE 0.057 → calibrated 0.003; held-out 0.003) and has a promotion record — but no active handler-selection path is wired, so it stays record-only; **L3 narration verification** was **redesigned from measurement**: the three-level `groundedness` `score` (which over-credited a narration restating a single committed fact) is replaced by one coverage `noul` and one contradiction `noul` per committed fact plus the aggregate hazards, with coverage computed in code as reflected-facts/total-facts. On a 25-case corpus it now **passes** (51 decisive verdicts, 94.1% verdict accuracy, calibrated Brier 0.057, ECE 0.056) with zero false accepts and 100% accept accuracy, and carries a promotion record; the single miss (c3) is a flag-label mismatch on a contradiction the lane still refused to accept, and there is no active narration path, so the record is evidence, not activation. A promotion-status section in `scripts/report-system-one-decisions.ts` now reports every lane's promoted/recorded state. The **L4 memory-reranking primitive** was evaluated against the Plan 3 provider-free recall oracle: across 81 live reranks the fused order left recall@K/MRR/nDCG unchanged on the supported corpus (every required source was already inside top-K), improved the best required-source position on both alias expected-miss cases (+2 ranks) without demoting any, and its 30 decisive calls scored **100%** case accuracy with raw ECE 0.135 → calibrated **0.0036**, so it carries an evidence-only promotion record. The 6 eligible label misses were exactly the alias cases where the lane deferred at ~0.4 confidence, the acted subset has no observed errors, and the lane is now wired in **shadow (record-only)** into the adventure planning recall path (a live turn recorded one structured `memory-reranking` decision with a two-candidate shortlist while the provider path kept the recall order). The **L2 adventure-selection** and **L6 guardrails** primitives now exist with live benchmarks. Adventure-selection, re-derived under the production-shaped gate protocol (no `uid`) over 30 frozen cases plus 86 harvested cases (1 human-confirmed, 85 agent-reviewed from synthetic play; 348 live calls), named a candidate on 54 calls with every pick acceptable; at the server default 0.75 only 6 acted, and a sweep recommends **0.40** (review <= 0.40), where it passes with 54 acted, **100%** accuracy, calibrated Brier 0.0002 and ECE 0.0110, and it carries an evidence-only record (threshold selected on the same corpus). Decision stability was 100% (0/116 conflicted). Agent-reviewed cases are scored and reported separately and never gate the record. It is wired into fresh adventure turns: **shadow (record-only)** except for `exact_srd_check.select`, which can commit when the lane is promoted and its lane mode is `active` — the first active path was observed live on 2026-09-18 with three DC-15 check commits. The human harvested case is the live deferral on a "drop my longsword" declaration whose binding was an `unequip` candidate — a stable 3/3 coverage miss, while the provider path still commits the right action. Guardrails first acted at only **90.5%** accuracy against the strict 0.95 gate — every error a false positive on fiction or meta questions (a hint request, an out-of-character question, in-character dialogue, a fictional threat scored as severe). The hazard criteria were redesigned from that measurement (override addresses the assistant itself, disclosure demands protected material rather than story hints, severity judges the real user rather than fictional drama) and the corpus was expanded to 56 independent cases; the lane then caught every hazard with **100%** acted accuracy (75 acted, calibrated ECE 0.0022; 100% decision stability, 0 of 57 cases conflicted, in a production-shaped 171-call re-run that includes one confirmed harvested live case) and carries an evidence-only record. It is now wired in **shadow (record-only)** into the room-turn route (raw content, hazards mirrored to `selection.flags`); a live room turn recorded a benign message as `fallback`/`pass`. The deterministic policy checks are a permissive stub and the lane is not a moderation guarantee. Remaining: build a shadow-data review path so live rejected beats can become negative examples and re-validate the Director and narration calibration against them; carry the redesigned L3 per-fact decomposition into the remaining unwired lanes; keep the promoted routing lane under observation.
- **W2–W6 planned:** confidence-gated lanes (Director, adventure selection, routing, advisory narration verification, reranking, guardrails) with deterministic fallback, under the same shadow-evaluate-promote discipline. Design and invariant: [Jev integration](jev-integration.md). No lane may mutate state or become load-bearing.
