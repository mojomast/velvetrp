...
- **M5.2 In Progress/partial (delivered at the historical v45r1 checkpoint):** repository administration and the fixed-local-owner HTTP/client transport are delivered. Owner/GM can authoritatively GET one management projection and POST the closed create-companion/create-grant/revoke-grant command union; writes atomically commit revision, idempotency, receipt, and audit state and return only a safe receipt. v45 replaced v44 sidecars without losing rows and uses durable principals for history, while fresh authorization remains repository-derived from current owner/GM state. This is not a current v45 support claim. There is no companion UI, grant exercise, dismissal, proposal/decision administration, or public member HTTP projection, so exposure does not close the milestone.
...
- **Next task after drift remediation:** scope the closed declarative rules IR milestone with exact consumers, contract boundaries, current-schema impact, and exclusions before implementation. Live exact-candidate selection, optional companion management UI, and M5.5 persistence remain later work until separately promoted.
