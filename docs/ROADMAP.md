...
- **W1 in progress:** the L1 Director selector is wired in **shadow mode** (record-only, behavior unchanged), the evaluation harness now grades predicted probabilities with Brier/ECE, and decision records are readable through a bounded `no-store` API. Remaining: accumulate shadow data on real sessions, set per-lane thresholds from that data, then promote a lane only when it clears its oracle-agreement/calibration gate.
