...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v48r1, with supported canonical populated v46/v47->v48 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 102 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
- **Validation:** the current deterministic E2E suite passes all 16 cases, including companion create/grant/revoke/replay, one consumable response-after-commit reload reconciliation against a coherent custom immutable published pack, and provider-committed travel receipt reload. Deterministic fixture changes remain harness-only; catalog attestation is unchanged.
...
- **Next tasks:** live candidate generation/selection HTTP and client selection remain later slices. M5.2 may still add a minimal companion management UI; M5.5 persistence/command/client integration remains Planned.
- v48 is current; canonical populated v46/v47 upgrades are supported and v45 and earlier are unsupported. Provider/adventure exact travel tool selection, immutable binding/accounting, crash recovery, safe narration, HTTP/client receipt display, and provider-committed travel E2E are delivered. Live candidate generation/selection HTTP and client selection remain absent. HTTP remains at 102 operations. The protected catalog remains unchanged.
