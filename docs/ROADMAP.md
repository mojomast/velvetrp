...
This roadmap is the sole owner of current milestone status and remaining scope. It grew from the completed gap analysis in [the historical RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md). Current persistence is schema v53r1, populated v46-v52 startup upgrades are supported, repository modules live under `server/src/repo/`, shared runtime contracts live under `packages/contracts/src/`, and the checked API inventory contains 111 counted trusted-local RPG HTTP operations plus feature discovery.
...
- **Next task after drift remediation:** scope the closed declarative rules IR milestone with exact consumers, contract boundaries, migration impact, and exclusions before implementation. Live exact-candidate selection, optional companion management UI, and M5.5 persistence remain later work until separately promoted.
- **Current baseline:** v53r1; populated v46-v52 upgrades; 111 counted operations plus feature discovery. Live exact-candidate generation/selection HTTP and client selection remain absent. Existing manual travel remains separate. The protected catalog is unchanged.
