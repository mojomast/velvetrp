...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v45r1, with supported canonical populated v43/v44->v45 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 102 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
After H0.4, parallel tracks are:
  ...
  M5.4 exact-candidate v1 checkpoint Complete -> semantics freeze -> v46 allocation + persistence/travel adapter
...
- **M5.2 In Progress/partial (v45r1):** repository administration and the fixed-local-owner HTTP/client transport are delivered. Owner/GM can authoritatively GET one management projection and POST the closed create-companion/create-grant/revoke-grant command union; writes atomically commit revision, idempotency, receipt, and audit state and return only a safe receipt. v45 replaces v44 sidecars without losing rows and uses durable principals for history, while fresh authorization remains repository-derived from current owner/GM state. There is no companion UI, grant exercise, dismissal, proposal/decision administration, or public member HTTP projection, so exposure does not close the milestone.
...
- **M5.4 checkpoint Complete (no schema):** the exact-candidate v1 contract and vectors are frozen with first and only current kind `actor.travel`, provider-safe projections, exact lifecycle/quote/decision/supersession/expiry/receipt bindings, and fail-closed execution gates. Persistence and adapters remain Planned. Before v46 allocation, freeze stable turn-scoped `connectionId`, a single-actor party, no-confirmation policy, behavior when more than 32 routes are available, expiry/supersession, and receipt mapping.
...
- **Next tasks:** freeze the unresolved M5.4 travel semantics before allocating v46: stable turn-scoped `connectionId`, a single-actor party, no-confirmation policy, behavior for more than 32 routes, expiry/supersession, and receipt mapping. M5.2 may still add a minimal companion management UI; M5.5 persistence/command/client integration remains Planned.
- v45 is the current allocated schema. The two companion and three consumable operations bring the explicit total to 102. The protected catalog remains unchanged.
