...
- **H0.4 Complete:** root `npm run health` runs exactly `npm run typecheck`, `npm run build`, `npm test`, and `npm run test:e2e` once each in that order. The final recovery run used a unique `/dev/shm` `TMPDIR` and passed all four phases in order. The first attempt timed out externally; the second exposed a global foreign-key preflight regression, which was corrected and scoped before the final wrapper passed. Independent release/security review found no material issues and confirmed deterministic/live credential isolation. Commits: `3e6c1a6 fix(repo): scope migration integrity preflight`; `f18a081 chore: add canonical health gate`; `7286ded docs: document canonical health gate`; docs/status commit remains pending. Wave H/H0 is green; M5.1 is now In Progress but not delivered, and all other M5 milestones remain Planned.

### Build next — M5

- **M5.1 In Progress (not delivered):** the read-only start recheck confirms provisional v43r1 is available, with no contract/code/schema drift and no blockers. Presence writes reject stopped sessions; stopped-session projections are structurally historical at-stop. Detach blocks only a running session with live-present rows and preserves historical presence. Player location is the existing principal-visible location union, never actor-inferred. Commands use a session-root `expectedRevision`; per-NPC revisions are informational. Authority and transitions execute in one immediate repository transaction; DDL enforces structural integrity only.
...
- **Next exact operation:** parallel disjoint new contract files/tests and sole schema-steward v43 migration/fixture work. Shared barrels/wiring are reserved for the integration owner, and protected `server/src/repo/contentCatalogRepo.ts` remains untouched. This start record makes no shipped or feature-validation claim.
