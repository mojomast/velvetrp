...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v46r1, with supported canonical populated v44/v45->v46 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 102 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
- **M5.4 In Progress/partial (v46r1):** exact-candidate v1 fixes server-owned scope `connectionId` as `adventure-turn:<turnId>`, exactly `[scope.actorId]`, no confirmation, no quote, and fail-closed batches above 32. v46 delivers exactly five persistence categories: batches, candidates, explicit supersessions, expiration observations, and canonical layout attestation. Public actor and legacy travel now share one authoritative policy and connection-scoped transaction core; the actor wrapper replays an exact stopped-session command before resolving one fresh running session, while observers are denied and GM travel, discovery, reputation, cardinality, and rollback behavior remain authoritative. The candidate generator requires the exact adventure-turn principal, current authority, one running session, one actor, location, and world revision; it emits at most 32 deterministic legal routes, persists exact empty batches, and replays persisted batches before world, clock, or ID work. Decisions, selection/execution, and receipt links are absent; receipt linking requires a later additive schema. No provider/tool advertising, candidate route/client surface, or candidate E2E was added. Existing manual world travel remains exposed as before and is not candidate-backed.
...
- **Validation:** deterministic E2E passed all 15 cases, including companion create/grant/revoke/replay and one consumable response-after-commit reload reconciliation against a coherent custom immutable published pack. Deterministic fixture changes remain harness-only; catalog attestation is unchanged.
- **M5.4 commits and validation:** persistence commits are `e0ef5f8`, `a690e3a`, and `71393a9`; delivered travel prerequisites are `bd921b8 refactor(repo): unify authoritative travel execution` and `c05a289 feat(repo): generate exact travel candidates`. The latest focused server aggregate passed 104 tests and server typecheck. No E2E was run because candidate execution and exposure boundaries were not added.
- **Next tasks:** design and allocate the later additive receipt-link schema together with atomic candidate selection and world-adapter execution. Adventure/provider integration follows; provider/tool advertising, candidate routes, client, and E2E remain later slices. M5.2 may still add a minimal companion management UI; M5.5 persistence/command/client integration remains Planned.
- v46 is current; canonical populated v44/v45 upgrades are supported and v43 is unsupported. HTTP remains at 102 operations. The protected catalog remains unchanged.
