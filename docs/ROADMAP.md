...
After H0.4, parallel tracks are:
  ...
  M5.3 combat-health Slice 0 -> consumable contract prerequisite delivered -> atomic runtime Slice 1
...
- **M5.3 In Progress/partial (no schema):** Slice 0 makes active encounter HP authoritative, mirrors actor health atomically through M15, fails external health mutation closed, advances round effects only at wrap, and retains concentration until replacement/removal. The consumable contract prerequisite is also delivered but not wired/live: it binds exact item, inventory entry, target, effect plan, request digest, expected M15 revisions, and result evidence. Policy admits category `consumable`, quantity one, damage/healing/resource health-guard-focus effects, and instant modifiers; it explicitly excludes spell-slot resource effects because no spell level is available, plus conditions, non-instant effects, and nonconsumables. Current route unions remain unchanged. Atomic runtime Slice 1 remains next.
...
- **Latest Wave A commits:** `ff2610c feat(contracts): define combat consumable protocol`; `42d9934 feat(repo): add v45 companion principals`; `839f70f feat(repo): administer companion grants`.
- **Next exact operation:** implement the atomic connection-scoped consumable planner/executor. Add M5.2 HTTP administration contracts, routes, and client only after preserving that repository boundary. M5.4 persistence/travel-adapter and M5.5 persistence/command/client integrations remain Planned and gated by their domain designs.
- v45 is the current allocated schema. The M5.3 consumable prerequisite and M5.4/M5.5 checkpoints added no HTTP operations; the explicit total remains 97. The protected catalog remains unchanged.
