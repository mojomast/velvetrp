...
After H0.4, parallel tracks are:
  ...
  M5.4 exact-candidate v1 checkpoint Complete -> v46 persistence foundation -> travel adapter
...
- **M5.4 In Progress/partial (v46r1 persistence foundation):** exact-candidate v1 fixes server-owned scope `connectionId` as `adventure-turn:<turnId>`, exactly `[scope.actorId]`, no confirmation, no quote, and fail-closed batches above 32. v46 delivers exactly five persistence categories: batches, candidates, explicit supersessions, expiration observations, and canonical layout attestation. Repository operations reconstruct all relational/envelope/lifecycle bindings, replay exact batches without consuming time/IDs, and require the exact scoped principal to retain current owner/GM/controller authority; unrelated members/observers are masked before integrity. Expiry is always derived from server time; no replacement means natural expiry/staleness, never cancellation. Decisions and receipt links are absent and require a later additive execution schema. Backfill and normal application tables are empty because no generator, execution, world adapter, provider advertising, route, client, or E2E was added.
...
- **M5.4 commits and validation:** `e0ef5f8 feat(contracts): freeze travel candidate persistence`; `a690e3a feat(repo): add v46 exact candidates`; `71393a9 feat(repo): persist exact travel candidates`. Final focused aggregates passed 38 contract tests and 83 server candidate/migration/support/repository tests; contract and server typechecks passed. No E2E was run for this persistence-only slice.
- **Next tasks:** M5.4 still needs the separately reviewed connection-scoped authoritative world travel candidate adapter, generator, and execution design. Execution receipt linking requires a later additive schema; provider, route, client, and E2E slices remain later. M5.2 may still add a minimal companion management UI; M5.5 persistence/command/client integration remains Planned.
