...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v47r1, with supported canonical populated v45/v46->v47 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 102 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
- **M5.4 In Progress/partial (v47r1):** v46 owns exact issuance and lifecycle history; v47 adds one immutable attested execution-link layout and repository-only atomic exact actor travel. Replay requires current exact principal authority, reconstructs the original batch, and cryptographically verifies selection, linked envelope, result, and complete deterministic world evidence while surviving expiry, supersession, stop, and later world changes. No decision tables, provider/tool advertising, candidate route/client surface, or candidate E2E were added. API remains 102 and existing manual world travel remains exposed.
...
- **M5.4 commits and validation:** persistence commits are `e0ef5f8`, `a690e3a`, and `71393a9`; delivered travel prerequisites are `bd921b8 refactor(repo): unify authoritative travel execution` and `c05a289 feat(repo): generate exact travel candidates`. The repository-only v47 execution/link slice passed 41 exact-candidate contract tests and 126 server migration/support/candidate/world/repository tests plus contract/server typechecks. No E2E was run because provider and product exposure boundaries were not added.
- **Next tasks:** adventure-turn/provider integration is next; provider/tool advertising, candidate routes, client, and E2E remain later slices. M5.2 may still add a minimal companion management UI; M5.5 persistence/command/client integration remains Planned.
- v47 is current; canonical populated v45/v46 upgrades are supported and v44 and earlier are unsupported. Repository-only exact travel selection/execution and immutable links are delivered; provider/tool, routes, client, and E2E remain absent. HTTP remains at 102 operations. The protected catalog remains unchanged.
