...
After H0.4, parallel tracks are:
  ...
  M5.3 combat-health Slice 0 -> consumable runtime + HTTP/client UI delivered -> instant-modifier decision
  M5.4 exact-candidate v1 checkpoint Complete -> persistence/travel adapter gated
...
- **Latest Wave A commits:** `409103e feat(api): add companion administration lane`; `77505fb feat(app): expose companion and consumable actions`; `bab31cc fix(repo): share consumable action authority`; `441defc test(e2e): cover companion and consumable flows`, plus the earlier contract, schema, repository, and rollback commits.
...
- **Next tasks may proceed in parallel:** M5.2 may add a minimal companion management UI. M5.3 must decide and freeze instant-modifier semantics or explicitly narrow the contract. M5.4 persistence/travel-adapter and M5.5 persistence/command/client integrations remain Planned and may proceed in parallel under their domain designs.
