...
- **Next exact operation:** launch parallel, disjoint, read-only rechecks for M5.2 companion core, M5.3 atomic combat, M5.4 candidate protocol, and the M5.5 dice threat-model checkpoint.
- No schema beyond delivered v43 is allocated. M5.2 uses `vNext`, and every later persistence milestone receives `vNext` from its schema steward only at milestone start; each allocation moves the tested current-minus-two support window.
