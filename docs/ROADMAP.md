...
- **W1 in progress:** the L1 Director selector is wired in **shadow mode** (record-only, behavior unchanged), the evaluation harness now grades predicted probabilities with Brier/ECE and a Director calibration eval runs the selector against the frozen oracle, decision records are readable through a bounded `no-store` API and a shadow-report CLI, and the L3 narration-verification and L7 cost-router **primitives** are implemented as pure, unwired modules. L3 narration-verification and L7 cost-router are now wired in shadow (record-only). The [live Director calibration](system-one-director-calibration.md) sweep is the first gate verdict and it is **not ready**: after making the aggregate best-pick require action-level confidence (Director beats mutate state), the selector defers on every default-threshold sample, and the best development operating point (~0.70) reaches only ~65% agreement with the intended action — no threshold clears the 0.90 accuracy floor. The blocker is therefore **question/oracle design**, not a threshold tweak: the per-candidate support battery does not separate legal-and-preferred beats from merely legal ones strongly enough. Remaining: redesign the Director battery (e.g. atomic legality vs relevance vs priority signals) and/or grade against a weighted candidate preference, then re-measure; keep accumulating shadow data on real sessions; re-derive per-lane gates from that data; promote a lane only when it clears its gate.
