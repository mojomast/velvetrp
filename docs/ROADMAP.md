...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v44r1, with supported canonical populated v42/v43->v44 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 97 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
After H0.4, parallel tracks are:
  M5.1 NPC presence -> M5.2 foundation delivered -> repository administration core
  M5.3 combat-health Slice 0 delivered -> consumable prerequisites/policy
...
- **M5.2 In Progress/partial (v44r1):** shared contracts and an additive empty companion foundation are delivered. v44 provides immutable revisioned commands/receipts plus companion, presence-link, proposal, decision, grant, revocation, audit, and attestation sidecars with empty backfill. Repository administration commands/projections, routes, UI, and grant exercise are not delivered; delegated exercise and principal-specific UI/E2E still wait for L5.
- **M5.3 partial, Slice 0 delivered (no schema):** active encounter HP is authoritative; actor health mirrors atomically through M15; health-mutating external commands fail closed while an encounter is active; round-duration effects advance only at round wrap; concentration persists until replacement/removal with no damage-break rule. Consumable Slice 1 remains Planned and blocked on exact inventory/action transaction contracts.
...
- **Wave A commits:** `9efd346 feat(contracts): define companion core`; `7c41064 feat(repo): add v44 companion foundation`; `a2b479c feat(contracts): freeze exact candidate protocol`; `3d834f4 feat(server): freeze verifiable dice protocol`; `68b3c57 feat(repo): compose authoritative combat health`.
- **Next exact operation:** continue the M5.2 repository administration core and settle M5.3 consumable prerequisites/policy, especially the exact inventory/action transaction contracts. Keep M5.4 persistence/travel-adapter work and M5.5 persistence/command/client integration gated by their respective domain designs rather than treating checkpoint completion as implementation readiness.
- v44 is the current allocated schema. M5.3 Slice 0 and the M5.4/M5.5 checkpoints added no schema or HTTP operations. The protected catalog remains unchanged.
