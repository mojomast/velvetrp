...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v45r1, with supported canonical populated v43/v44->v45 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 97 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
After H0.4, parallel tracks are:
  M5.1 NPC presence -> M5.2 repository administration delivered -> HTTP/client administration
  M5.3 combat-health Slice 0 -> consumable contract -> repository atomic runtime delivered -> route/client wiring
...
- **M5.2 In Progress/partial (v45r1):** repository-only administration is delivered. Owner/GM can create a companion from a persisted present NPC in an attached running session and create or revoke bounded grants. Management and member public projections are role-safe; each fresh command atomically commits revision, idempotency, receipt, and audit state, and exact issuer replay survives demotion or membership removal. v45 replaces v44 sidecars without losing rows and uses durable principals for history, while fresh authorization remains repository-derived from current owner/GM state. Routes, UI, dismissal, proposal/decision administration, and grant exercise are not delivered.
- **M5.3 In Progress/partial (no schema):** Slice 0 makes active encounter HP authoritative, mirrors actor health atomically through M15, fails external health mutation closed, advances round effects only at wrap, and retains concentration until replacement/removal. Repository-only atomic consumable execution is now delivered for exact pinned category `consumable`, quantity one. One immediate transaction preflights exact catalog/inventory/target/combat/M15/effect-plan/ID state before RNG; resolves ordered damage, healing, and health/guard/focus resource effects; consumes one item and the acting action; aggregates acting and distinct-target M15 revisions; keeps actor-backed/combat HP coherent; performs round-wrap M16 duration work; and seals turn, log, events, and receipt. Exact immutable replay, role-safe separate legal-action/result readers, cardinality checks, rollback boundaries, and reopen validation are covered. Instant modifiers remain fail-closed despite contract eligibility because authoritative runtime semantics are absent; conditions, non-instant effects, spell-slot effects, and nonconsumables remain excluded. No route/client or live combat-union wiring is delivered, so the API remains 97 operations.
...
- **Latest Wave A commits:** `ff2610c feat(contracts): define combat consumable protocol`; `42d9934 feat(repo): add v45 companion principals`; `839f70f feat(repo): administer companion grants`; `e27402d feat(repo): settle combat consumables atomically`; `cd885dc test(repo): align v45 corruption assertions`.
- **Next tasks may proceed in parallel:** deliver M5.2 HTTP administration contracts, routes, and client adding two operations; wire M5.3 routes/client for only the supported repository consumable subset. Do not wire instant modifiers. M5.4 persistence/travel-adapter and M5.5 persistence/command/client integrations remain Planned and gated by their domain designs.
- v45 is the current allocated schema. M5.3 repository execution and the M5.4/M5.5 checkpoints added no HTTP operations; the explicit total remains 97. The protected catalog remains unchanged.
