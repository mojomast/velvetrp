...
- **Current baseline:** one disposable development schema with delete/recreate changes except for the exact tactical-map and campaign-director predecessor upgrades described in [Operations](operations.md#data-directory-and-current-schema); 145 counted operations plus feature discovery. Live exact-candidate generation/selection HTTP and client selection remain absent. Existing manual travel remains separate. The protected catalog is unchanged.
