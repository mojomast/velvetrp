...
- **M5.3 In Progress/partial (no schema):** Slice 0 makes active encounter HP authoritative, mirrors actor health atomically through M15, fails external health mutation closed, advances round effects only at wrap, and retains concentration until replacement/removal. The separate consumable lane exposes authoritative action GET, command POST, and exact-result GET for exact pinned category `consumable`, quantity one. One immediate transaction resolves ordered damage, healing, and health/guard/focus effects and atomically settles inventory, action, combat/actor health, revisions, turn, log, events, and receipt. The client renders the exact server target and action cost, persists an ambiguity marker before POST, never automatically retries, and reconciles exact result plus combat/log/actions across reload. Commit `87d53ab` closes the outstanding modifier decision by making all consumable modifier durations contract-ineligible: instant semantics are unavailable and noninstant modifiers are unsupported, with no modifier descriptor, settlement, legal action, or runtime path. No successful historical consumable modifier result exists. Shared catalog/power modifier contracts are unchanged and powers retain receipt-only instant behavior. Conditions, spell slots, other unsupported effects, and nonconsumables remain excluded, and the legacy combat union is unchanged. The decision is closed, but M5.3 remains partial unless its other milestone scope is complete.
- **M5.4 provider/adventure bridge and receipt display delivered (v48r1):** eligible controlled noncombat player turns idempotently generate provider-safe travel candidates before dispatch. A strict server-selected tool accepts only the exact selection response; successful provider call/tool call/round/selection/execution are immutably bound and recover without duplicate travel. The existing command-receipt GET and CampaignPlay receipt card now expose only a verified player-safe destination display name, one world revision advance, and commit time. Deterministic E2E covers one provider-committed travel and reload without a second move. No candidate generation/selection route, browser selector, POST owner, or local ambiguity marker was added.
...
- **Current baseline:** one disposable development schema with delete/recreate changes; 111 counted operations plus feature discovery. Live exact-candidate generation/selection HTTP and client selection remain absent. Existing manual travel remains separate. The protected catalog is unchanged.
