...
This roadmap turns the completed gap analysis in [the RPG integration plan](rpg-integration-plan.md) and [the 2026 architecture notes](roleplay-architecture-2026.md) into dependency-ordered work. Current persistence is schema v43r1, with supported canonical populated v41/v42->v43 startup upgrades, repository modules under `server/src/repo/`, shared runtime contracts under `packages/contracts/src/`, and 97 explicit trusted-local RPG HTTP operations under `server/src/routes/rpg/v1/`.
...
After H0.4, parallel tracks are:
  ...
  M5.5 threat-model checkpoint Complete -> commit-reveal dice Planned
...
- **M5.2-M5.6 Planned:** the completed read-only M5.2-M5.5 rechecks froze the next contracts but delivered no implementation and allocated no schema. M5.2 companion core, M5.3 atomic combat, M5.4 exact candidate protocol, M5.5 commit-reveal implementation, and M5.6 per-family adapters remain Planned. The M5.5 threat-model checkpoint alone is Complete. Delegated grantee exercise and principal-specific UI/E2E wait for remote identity/grants. Single active encounter remains mandatory.
...
- **Completed read-only rechecks:** M5.2 is contract-frozen and ready as a companion sidecar over NPC identity and v43 presence, with companion-owned proposals/decisions and administrable grants that are never exercisable before L5. M5.3 Slices 0/1 freeze active-encounter HP, duration, concentration, and replay boundaries; their no-schema item subset is a fixed combat `use consumable` command that spends the acting turn's action by server policy, not catalog `actionCost`, and accepts only an exact pinned item with category `consumable`, quantity >= 1, and effects limited to damage, healing, resource, or a modifier with `instant` duration. Condition, non-instant, persistent, and Waylamp effects/items are excluded. M5.4 is ready for a pure contract/protocol freeze with generic exact candidates, provider-safe projection, exhaustive kind policy, lifecycle links, and connection-scoped adapter requirements. M5.5's threat-model decisions and primitive-selection checkpoint are Complete; the cryptographic protocol and implementation remain Planned.
- **Next exact operation:** preserve parallelism: start M5.2 schema/contracts with its schema steward allocating v44, start the conditional no-schema M5.3 Slices 0/1 subset in disjoint files, and run only M5.4 pure contract work plus M5.5's exact versioned binary framing, domain separation, HKDF/HMAC inputs, rejection/timeout protocol, and vectors, with no additional schema and one integration owner for shared files.
- No schema beyond delivered v43 is currently allocated. The next schema steward may allocate v44 only when M5.2 implementation starts; M5.3 remains no-schema only while limited to the fixed subset above, and M5.4/M5.5 pure contract/vector work allocates no schema. The protected catalog remains unchanged.
