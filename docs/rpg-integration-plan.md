...
- **Character**: Velvet's durable persona, voice, boundaries, memories, and safe word.
...
1. Deterministic permission, safety, and safe-word rules.
...
- Permission, boundary, safe-word, or policy editing.
...
Factory/UoW `getCampaignRoomLinkingSnapshot` uses one explicit-column path-bound statement. A materialized SQL gate requires actor principal ancestry, recognized role, owner-pointer agreement for an owner actor, and canonical exact sole parent-backed owner membership plus `owner_role` before any attached/eligible candidate or title/participant presentation materializes. Campaign owner, sole owner-membership, and owner-parent IDs must each be SQLite text, binary-equal, NUL-free strict 1–128-character resource IDs in that gate. Strict JS validation remains after authorization, outsiders remain masked, and only the exact owner receives running globally unattached eligibility. Stopped attachments remain, cross-campaign attachment corruption is isolated, and selected participant ancestry corruption is loud. SQL selects only title, participant names, timestamps, and minimum state/ancestry evidence—never character IDs, boundaries, safe word, messages, context, preset, stop reason, or other private session fields. `attachCampaignSession` verifies exact requested-campaign authority under its immediate transaction, then classifies the attachment relation before reading the target session graph: foreign links always return one generic typed conflict regardless of foreign corruption, while same-campaign idempotency and unattached insertion require complete local graph validation.
...
Intact owner/GM/player/observer memberships are allowed. Authorization masks stale purported owners, outsiders, missing state, and cross-campaign targets as literal null; exact owner integrity and attributable aggregate/content/resource corruption are loud after authorization. Persona/sheet/actor one-to-one and campaign ancestry, exact sealed pins/profile ancestry, race/background/class/choice definitions, raw timestamps/types/integers, duplicate positions/identities, collection MAX+1 bounds, and resource actor ancestry are validated. Every campaign-attributable private-state row also requires exactly one same-campaign controller membership backed by an intact principal, an eligible owner/GM/player role, and a strict canonical timestamp; missing, orphaned, foreign-only, or malformed controller ancestry is loud only after authorization. Persisted content supplies display metadata. Ordered arbitrary technical IDs are validated but represented only as exact one-based positional labels, with category-specific proficiency prefixes. Controller/private notes, boundaries, safe word, audit, command, and unrelated persona fields are neither selected nor emitted; poisoned private payloads do not affect the read.
