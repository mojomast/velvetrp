...
- Milestones **M1-M3, M4.1-M4.6, and M5.1 are complete**. M5.2 remains In Progress/partial after its authoritative companion management GET/closed command POST and client transport because no companion UI, grant exercise, dismissal, or proposal/decision administration exists; its v45 delivery wording is historical, not current support. M5.3 remains In Progress/partial after its separate consumable action/command/exact-result lane and exact client UI/reconciliation flow because the legacy combat union is unchanged and other milestone scope remains. M5.4 is In Progress/partial with its v46 persistence foundation, shared authoritative travel policy/transaction core, explicit session adapter and public wrappers, and authoritative exact travel candidate generator delivered. Candidate selection/execution, receipt links, provider/tool advertising, candidate routes/client, and candidate E2E are absent; existing manual travel remains exposed and is not candidate-backed. M5.5 has completed only its pure protocol checkpoint. Remaining integration is ordered and statused only by [ROADMAP.md](ROADMAP.md).
