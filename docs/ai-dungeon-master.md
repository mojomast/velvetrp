...
## Coverage: any target, any attempt, any turn

All three paths keep the same authority rule: prose never establishes mechanics, and every committed change has a receipt.

- **Any target can be attacked.** An explicit player attack declaration naming a visible campaign actor or NPC materializes and starts an encounter through the normal lifecycle. Actor targets use their own combat state; NPC targets resolve to the nearest SRD stat block through a deterministic tier heuristic (common folk use the weakest humanoid profile, guards sit above them, and scouts, soldiers, adventurers, mages, priests, and leaders map to progressively stronger profiles). Templates pin on demand, and the attacked NPC's public name is carried as an immutable display label on the tracker and map token instead of the borrowed template name. Combat entry uses no provider planning and invents no statistics.
- **Entering combat always produces a tactical map.** Encounter start generates exactly one deterministic combat map bound to the encounter, reused on replay, with combatant tokens placed automatically and a neutral or terrain-inferred kind.
- **Combat controls stay front and center.** While an encounter is active the play surface shows a combat command bar with the round, whose turn it is, and the encounter-level controls: End turn, Run enemy turn (owner/GM), Complete encounter (owner/GM), and Open combat. Combat never completes by itself when the last enemy falls. The bar marks the terminal state (no current turn and a single living side) and the owner/GM completes the encounter there, which settles reward bundles for explicit claim.
- **Any concrete attempt becomes a check.** A conservative declaration-to-skill mapping ranks the matching SRD skill (Athletics, Investigation, Perception, the social skills, and the rest) ahead of unrelated candidates. When the provider deliberately holds on a strong-mapped attempt, the server commits the matching Medium/normal check deterministically with recorded server-fallback provenance. Questions, greetings, meta talk, pure travel, and ambiguous cross-skill phrasing map to nothing and are never given an invented roll.
- **Any turn gets narration.** Turns with committed mechanics narrate only from receipts, with contradictory prose falling back to the deterministic result line. Held turns (conversation, questions, emotes, or an attempt the provider declined) are narrated through the existing narration derivative: a bounded conversation prompt built from the actor's exact location, public cast portrayals, and public context, explicitly barred from asserting movement, items, damage, discovery, quest or progression state, attitude changes, or dice. Unsupported claims and provider failures keep the deterministic hold line, and the client auto-requests one retry per original turn under the auto-narrate preference. Impossible or meta requests remain narration-only and never mutate state.
