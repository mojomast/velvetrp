...
- `GET /api/provider/system-one` returns `{ id, providerType, enabled, laneModes, shadowAdventurePayload, baseUrl, model, hasApiKey, requestTimeoutSeconds, pricing, budget, confidencePolicy, confidenceCalibration, updatedAt }` and never returns the key.
- `PUT /api/provider/system-one` updates supplied fields, clamps `requestTimeoutSeconds` to 5-120, `pricing` to 0-1,000,000 USD per million, `budget.maxTotalTokens` to 1-1,000,000,000, `budget.maxRequestsPerWindow` to 1-100,000, and `budget.rateWindowMs` to 1,000-3,600,000, clamps each lane's `reviewThreshold` to at most its `actionThreshold`, and clamps `confidenceCalibration.<lane>.a` to 0-1,000 and `.b` to -1,000-1,000. `shadowAdventurePayload` selects only the adventure-selection shadow battery (`legacy` default, or the evaluation-only `shared-context`); an unknown value is ignored and a malformed persisted value fails closed to `legacy`, and neither variant changes the provider planning request or grants commit authority. The calibration is fit out of band by the lane evaluation and never changes which band a decision lands in.
