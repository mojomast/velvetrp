...
- `GET /api/provider/system-one` returns `{ id, providerType, enabled, baseUrl, model, hasApiKey, requestTimeoutSeconds, pricing, budget, confidencePolicy, confidenceCalibration, updatedAt }` and never returns the key.
