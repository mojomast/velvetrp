...
Repository refactor progress: `db.ts` migration modules and the initial campaign dependency, dice-command, core, actor-projection, actor-resource read, event-pagination, and administration adapter slices have been extracted behind compatibility facades. Campaign-administration access/projection, checkpoint/fork, recap, and timeline-history adapters now live in `server/src/repo/campaignAdmin/`; `campaignRepo.ts` and `campaignAdministrationRepo.ts` remain compatibility composition facades. Campaign import/export and gameplay event reconstruction/audit projections remain in their facades because their atomic, cross-domain dependencies have not yet been split. Public repository APIs, SQL, trigger logic, and migration order remain unchanged.
