...
Repository refactor progress: `db.ts` migration modules and the initial campaign dependency, dice-command, core, actor-projection, and administration adapter slices have been extracted behind compatibility facades. Campaign-administration checkpoint/fork, recap, and timeline-history adapters now live in `server/src/repo/campaignAdmin/`; `campaignRepo.ts` and `campaignAdministrationRepo.ts` remain compatibility composition facades. Campaign import/export and the remaining public timeline/event projections are still in their facades because their atomic, cross-domain dependencies have not yet been split. Public repository APIs, SQL, trigger logic, and migration order remain unchanged.
