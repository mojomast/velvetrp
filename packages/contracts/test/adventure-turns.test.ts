...
import {
  ...
  privateAdventureTurnSchema, providerCallMetadataSchema, roleSafeAdventureTurnSchema, toolProposalSchema,
...
describe("M1.10 adventure-turn contracts", () => {
  it("publishes exhaustive closed lifecycle vocabularies", () => {
    expect(adventureTurnStateSchema.options).toEqual(["declared", "proposed", "awaiting-confirmation", "mechanics-committed", "narrating", "completed", "cancelled", "failed"]);
    expect(narrationStatusSchema.options).toEqual(["none", "pending", "in-progress", "completed", "failed"]);
    expectTypeOf<AdventureTurnState>().toEqualTypeOf<"declared" | "proposed" | "awaiting-confirmation" | "mechanics-committed" | "narrating" | "completed" | "cancelled" | "failed">();
  ...
  it("structurally separates role-safe and private projections", () => {
    ...
    const privateTurn = { ...base, declaration: "I inspect the seal", toolCalls: [{ proposal, status: "proposed", receiptLinks: [] }],
