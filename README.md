...
Rules modules are pure deterministic code. Content is published as immutable, validated packs and campaigns bind to exact `(packId, packVersion)` identities. The current development SRD starter is `srd-5.1:starter@1.4.0+dc8659693163`; changing content means publishing a new exact version rather than rewriting history.
