...
Rules modules are pure deterministic code. Content is published as immutable, validated packs and campaigns bind to exact `(packId, packVersion)` identities. The current development SRD starter is `srd-5.1:starter@1.6.0+3a8566c00093`; changing content means publishing a new exact version rather than rewriting history.
...
The full operation contract is in the [API reference](docs/api.md): 161 counted explicit trusted-local RPG operations plus feature discovery, excluding implicit HEAD aliases. The implementation intentionally does not duplicate the route and schema tree here.
