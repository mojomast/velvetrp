...
export interface LevelUpWizardApi {
  ...
  apply: (campaignId: string, campaignCharacterId: string, input: CharacterProgressionHttpApplyRequest) => Promise<CharacterProgressionHttpApplyResponse>;
...
export function LevelUpWizard({ campaignId, campaignCharacterId, api, onUnavailable = () => undefined, onSheetRefreshed = () => undefined }: LevelUpWizardProps) {
  ...
  const load = useCallback(async (authoritative = false, retryFocus = false) => {
    ...
    try {
      ...
      setState(nextState); setPreview(nextPreview); setSelections([]); setLoading(false); setRefreshing(false);
  ...
  async function updatePreview() {
    const generation = ++generationRef.current; setError(""); setNotice("Calculating exact changes on the server…");
    try {
      const result = await api.preview(campaignId, campaignCharacterId, { selections });
      if (!mountedRef.current || generationRef.current !== generation) return;
      setPreview(result); setNotice("Exact advancement changes are ready for review."); focusStatus(generation);
  ...
  function choose(choiceId: string, ability: ProgressionSelection["ability"]) {
    ...
    setPreview((current) => current); setNotice("");
  }

  async function applyOnce() {
    if (!preview || applyLocks.has(key)) return;
    ...
    try {
      const result = await api.apply(campaignId, campaignCharacterId, input);
      ...
      setState(result.progression); setReceipt(result.receipt); setPreview(null); setNotice(`Levels ${result.receipt.appliedLevels.map((level) => level.level).join(", ")} applied exactly once. Authoritative sheet refreshed.`);
      sheetRefreshedRef.current(freshSheet); focusStatus(generationRef.current);
    } catch (applyError) {
      if (applyLocks.get(key)?.token !== token) return;
