...
export interface RetryGenerationInput { jobId: string; requestDigest: string; failedAttempt: number }
export interface GenerationRecoveryApi {
  openDraft: (draftId: string) => void;
  reconcileJob: (jobId: string) => void;
  retryFailedJob: (input: RetryGenerationInput) => void;
}
...
export function GenerationRecoveryPanel({ actorRole, drafts, jobs, busyJobIds = [], disabled = false, retrySupported = true, api }: GenerationRecoveryPanelProps) {
  const [retrying, setRetrying] = useState<DurableGenerationJob | null>(null);
  const [confirmedDigest, setConfirmedDigest] = useState(false);
  ...
  return <section className="admin-section" aria-labelledby="generation-recovery-heading">
    ...
    {!retrySupported && <p className="content-warning">Paid retry is unavailable from redacted recovery metadata. Open the exact durable draft or start a separately reviewed generation request.</p>}
    ...
    {retrying && <section className="command-review" aria-labelledby="generation-retry-heading"><h3 id="generation-retry-heading">Review failed generation retry</h3><p>This starts a new paid attempt for the same durable logical job. It does not replay on reload or on reconciliation.</p><dl className="command-detail-list"><div><dt>Job</dt><dd>{retrying.jobId}</dd></div><div><dt>Failed attempt acknowledged</dt><dd>{retrying.attempt}</dd></div><div><dt>Exact request digest</dt><dd><code>{retrying.requestDigest}</code></dd></div></dl><label className="checkbox"><input type="checkbox" checked={confirmedDigest} onChange={(event) => setConfirmedDigest(event.target.checked)} /> Confirm this exact digest and acknowledge failed attempt {retrying.attempt}</label><button className="primary" type="button" disabled={disabled || !confirmedDigest} onClick={() => api.retryFailedJob({ jobId: retrying.jobId, requestDigest: retrying.requestDigest, failedAttempt: retrying.attempt })}>Retry failed job once</button><button type="button" disabled={disabled} onClick={() => { setRetrying(null); setConfirmedDigest(false); }}>Cancel retry review</button></section>}
  </section>;
}
