...
function OutcomeReceipt({ result, labels }: { result: CombatActionCommandResponse; labels: Map<string, string> }) {
  ...
  return <section className="combat-receipt" aria-labelledby="combat-receipt-heading">
    ...
    <dl><div><dt>Action</dt><dd>{resolution.kind}</dd></div><div><dt>Acting</dt><dd>{name(resolution.actingCombatantId)}</dd></div><div><dt>Targets</dt><dd>{resolution.targetIds.length ? resolution.targetIds.map(name).join(", ") : "None"}</dd></div><div><dt>Revision</dt><dd>{receipt.revisionBefore} → {receipt.revisionAfter}</dd></div><div><dt>Round</dt><dd>{resolution.roundBefore} → {resolution.roundAfter}</dd></div><div><dt>Occurred</dt><dd>{receipt.occurredAt}</dd></div></dl>
     {resolution.outcomes.length > 0 && <ul>{resolution.outcomes.map((outcome, index) => <li key={`${outcome.kind}-${outcome.targetId}-${index}`}><strong>{name(resolution.actingCombatantId)} → {name(outcome.targetId)}:</strong> {outcome.kind === "damage" ? <>Damage: {outcome.applied} {outcome.damageType} · HP {outcome.hitPointsBefore} → {outcome.hitPointsAfter} · {outcome.statusBefore} → {outcome.statusAfter}{outcome.rulesetId && <> · {outcome.rulesetId} @ {outcome.rulesetVersion} · attack {outcome.attackRoll} + modifiers = {outcome.attackTotal} vs AC {outcome.armorClass}{outcome.critical ? " · critical" : outcome.hit ? " · hit" : " · miss"}</>}</> : outcome.kind === "survival" ? <>Survival: {outcome.roll ? `d20 ${outcome.roll} · ` : ""}{outcome.successes} successes, {outcome.failures} failures · {outcome.statusAfter}</> : outcome.kind === "status" ? <>Status: {outcome.statusBefore} → {outcome.statusAfter}</> : outcome.kind === "stand-up" ? <>Stand up: {outcome.movementCostFeet} ft of movement · prone cleared</> : <>Contest: {outcome.contest} · {outcome.success ? "success" : "failure"} · {outcome.attackerRoll} vs {outcome.defenderRoll}</>}</li>)}</ul>}
...
export function CombatTrackerPage({ api, campaignId, sessionId, actorRole = "gm", audience = "gm", controlledActorId, initialCombatId, onBack, onReturnToRoom, onUnavailable, focusHeadingRequest,
  ...
  return <Container disabled={embedded ? blocked : undefined} className={embedded ? "atlas-combat-embedded" : "combat-page"} aria-labelledby="combat-heading"><div className="combat-shell">
    ...
    {!observerWorkspace&&confirmed && <OutcomeReceipt result={confirmed} labels={labels} />}
    ...
    {gmWorkspace&&enemyTurnResult&&<OutcomeReceipt result={enemyTurnResult} labels={labels}/>}
