...
export function EncounterLifecyclePanel({ campaignId, sessionId, api, onCombatReady, onRewards }: { campaignId: string; sessionId?: string; api: EncounterLifecycleApi; onCombatReady: (combatId: string) => void; onRewards: (response: CombatEndCommandResponse) => void }) {
  ...
  const refresh = async (explicit = false) => {
    try {
      ...
      const sessions = sessionId ? available.sessions.filter((session) => session.sessionId === sessionId) : available.sessions;
      setEncounters(sessionId ? value.encounters.filter((entry) => entry.sessionId === sessionId) : value.encounters);
      setCandidates(available);
      setSelectedSessionId((current) => sessions.some((session) => session.sessionId === current) ? current : sessions[0]?.sessionId ?? "");
  ...
  const visibleSessions = candidates ? (sessionId ? candidates.sessions.filter((session) => session.sessionId === sessionId) : candidates.sessions) : [];
