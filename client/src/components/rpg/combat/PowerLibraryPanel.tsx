...
function PowerResult({ value }: { value: ActorPowerCommandResponse }) {
  return <section className="power-result" aria-labelledby="power-result-heading"><h3 id="power-result-heading">Confirmed power response</h3>
    <dl><div><dt>Power</dt><dd>{value.resolution.powerRef.definitionId}</dd></div><div><dt>Receipt key</dt><dd>{value.receipt.idempotencyKey}</dd></div><div><dt>Revision</dt><dd>{value.receipt.revisionBefore} → {value.receipt.revisionAfter}</dd></div><div><dt>Occurred</dt><dd>{value.receipt.occurredAt}</dd></div><div><dt>Costs</dt><dd>{value.resolution.costs.map(costText).join(", ") || "None"}</dd></div><div><dt>Targets</dt><dd>{value.resolution.targetIds.join(", ")}</dd></div></dl>
    ...
    <ul aria-label="Returned actor states">{value.actorStates.map((state)=><li key={state.actorId}><strong>{state.actorId}</strong> · revision {state.revision} · resources {state.resources.map((resource)=>`${resource.resourceId} ${resource.current}/${resource.capacity}`).join(", ")||"none"} · active effects {state.activeEffects.map((effect)=>effect.effectId).join(", ")||"none"}</li>)}</ul>
...
export function PowerLibraryPanel({ powers, loading = false, error = "", disabled = false, commandStatus = "", result = null, onRefresh, onUse }: PowerLibraryPanelProps) {
  ...
  return <section className="combat-panel power-library" aria-labelledby="power-library-heading" aria-busy={loading}>
    ...
    {powers && <><dl className="power-slot-list" aria-label="Server-returned spell slots">{powers.slots.map((slot) => <div key={slot.slotId}><dt>Level {slot.level} slot</dt><dd>{slot.current} / {slot.max}</dd></div>)}</dl>
      ...
      {selected && <section className="power-command-review"><h3>Server-planned command</h3>{selected.targeting !== "self" && <fieldset><legend>Valid server targets</legend>{selected.validTargets.map((target) => { const checked=targets.includes(target.actorId);return <label key={target.actorId}><input type={selected.targeting === "single" ? "radio" : "checkbox"} name="power-target" checked={checked} disabled={disabled||(!checked&&targets.length>=selected.maxTargets)} onChange={() => { setTargets((current) => selected.targeting === "single" ? [target.actorId] : current.includes(target.actorId) ? current.filter((id) => id !== target.actorId) : current.length<selected.maxTargets?[...current,target.actorId]:current); setReviewing(false); }} />{target.label ?? target.actorId}</label>;})}</fieldset>}
        {!reviewing ? <button type="button" className="primary" disabled={disabled || !selectionValid} onClick={() => setReviewing(true)}>Review power command</button> : <div className="action-review"><dl><div><dt>Power</dt><dd>{selected.powerRef.definitionId}</dd></div><div><dt>Submitted targets</dt><dd>{selected.targeting === "self" ? "None (server resolves self)" : targets.join(", ")}</dd></div><div><dt>Maximum targets</dt><dd>{selected.maxTargets}</dd></div><div><dt>Server costs</dt><dd>{selected.costs.map(costText).join(", ") || "None"}</dd></div><div><dt>Effect kinds</dt><dd>{selected.effectKinds.join(", ")}</dd></div><div><dt>Concentration</dt><dd>{selected.concentration ? "Required" : "No"}</dd></div></dl><button type="button" className="primary" disabled={disabled} onClick={() => onUse?.(selected, selected.targeting === "self" ? [] : targets)}>Execute once</button></div>}
      </section>}
