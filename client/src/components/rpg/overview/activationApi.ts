import { campaignRoomActivationReadinessSchema, campaignRoomActivationRequestSchema, campaignRoomActivationResponseSchema, campaignRoomParticipantRequestSchema, campaignRoomParticipantResponseSchema, resourceIdSchema, type CampaignRoomActivationRequest, type CampaignRoomParticipantRequest } from "@velvet/contracts";
...
export async function addCampaignRoomParticipant(campaignId: string, sessionId: string, input: CampaignRoomParticipantRequest) {
  const body = campaignRoomParticipantRequestSchema.parse(input);
  const result = campaignRoomParticipantResponseSchema.parse(await readResponse(await fetch(`${lane(campaignId, sessionId)}/participant-commands`, {
    method: "POST", cache: "no-store", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body),
  })));
  if (result.campaignId !== campaignId || result.sessionId !== sessionId || result.campaignCharacterId !== body.campaignCharacterId || result.receipt.idempotencyKey !== body.idempotencyKey) throw new Error("Participant receipt identity mismatch");
  return result;
}
