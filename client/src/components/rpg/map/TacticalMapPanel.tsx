...
function BoundTacticalMapPanel({ campaignId, sessionId, actorId, audience, mode, encounterId, combatantId, api, readOnly = false, refreshKey = 0, commandsBlocked = false, location, onRefreshLocation }: Props) {
  ...
  const [notice, setNotice] = useState(""); const [seed, setSeed] = useState(() => locationId ? `velvet-map:${locationId}`.slice(0, 256) : "velvet-map"); const [kind, setKind] = useState<"dungeon" | "cave" | "arena">("arena");
  ...
  return <section className="tactical-map-panel" aria-labelledby="tactical-map-heading"><header><h2 id="tactical-map-heading">{location?.name ?? "Tactical map"}</h2><span>{mode}</span></header>
    ...
    {["missing", "mismatch"].includes(state) && audience === "gm" && !accessReadOnly && <details open className="tactical-map-setup"><summary>{state === "mismatch" ? "Review replacement map" : "Prepare a map"}</summary><fieldset disabled={commandsBlocked || locationRefreshRequired || (state === "mismatch" && !location)}><legend>Generate deterministic map</legend><label>Layout<select value={kind} onChange={(event) => { setKind(event.target.value as typeof kind); setReplaceReviewed(false); }}><option value="arena">Arena</option><option value="dungeon">Dungeon</option><option value="cave">Cave</option></select></label><label>Exact seed<input value={seed} maxLength={256} onChange={(event) => { setSeed(event.target.value); setReplaceReviewed(false); }} /></label>
