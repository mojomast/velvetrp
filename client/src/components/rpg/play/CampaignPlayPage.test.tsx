...
describe("CampaignPlayPage", () => {
  ...
  it("opens mechanical possessions from Character and preserves a hidden review lock", async () => {
    const client = api();
    const actorToolsApi: RpgCharacterSheetApi = { getSheet: vi.fn(), getInventory: vi.fn().mockResolvedValue({ entries: [{ kind: "stackable", entryId: "rope", item: { kind: "item", packId: "pack", packVersion: "1", definitionId: "rope" }, quantity: 1 }], equipment: [], capacity: 10, revision: 4 }), getResources: client.getActorResources, getEffects: client.getActorEffects, getPowers: vi.fn().mockResolvedValue({ known: [], prepared: [], slots: [], uses: [], legalNow: [], legalCommands: [], revision: 4 }), getWallet: vi.fn().mockResolvedValue({ wallet: { balances: [] }, revision: 4 }), getShop: vi.fn(), inventoryCommand: vi.fn(), economyCommand: vi.fn(), rest: vi.fn(), checkCommand: vi.fn(), powerCommand: vi.fn(), spellCommand: vi.fn(), effectCommand: vi.fn(), resourceCommand: vi.fn(), getCampaignContent: vi.fn().mockRejectedValue(new Error("no catalog")), getCampaignPack: vi.fn() };
