...
export function MechanicReceiptCard({ campaignId, links, api }: MechanicReceiptCardProps) {
  ...
  useEffect(() => {
    let current = true;
    setLoads(Object.fromEntries(unique.map((link) => [link.commandId, { state: "loading" as const }])));
    for (const link of unique) void api.getCampaignCommandReceipt(campaignId, link.commandId).then(({ receipt }) => {
      if (current) setLoads((value) => ({ ...value, [link.commandId]: { state: "ready", receipt } }));
    }).catch(() => { if (current) setLoads((value) => ({ ...value, [link.commandId]: { state: "error" } })); });
