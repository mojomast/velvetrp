...
export interface ShopBrowserProps {
  ...
  disabled?: boolean;
  /** A caller that already knows the exact shop can hide the free-form lookup form. */
  showKnownShopForm?: boolean;
...
export function ShopBrowser({ wallet, shop, shopId, quote, currencies, disabled = false, showKnownShopForm = true, itemLabel, onLoadShop, onQuote, onPurchase }: ShopBrowserProps) {
  ...
  return <section className="actor-section shop-browser" aria-labelledby="shop-heading">
    ...
    {showKnownShopForm && <form className="known-shop-form" onSubmit={(event) => { event.preventDefault(); if (knownId) onLoadShop(knownId); }}>
