...
export function RpgCharacterSheetPage({ campaignId, campaignCharacterId, api, onBack, onUnavailable, focusHeadingRequest }: RpgCharacterSheetPageProps) {
  ...
  function submitEconomy(intent: EconomyIntent, operation: string, after?: (value: EconomyHttpCommandResponse) => void, refresh: () => Promise<void> = refreshWallet) {
    if (!wallet) return;
    ...
    void runCommand(operation, command, () => api.economyCommand(campaignId, actorId, command), (value) => receiptText(value.receipt), refresh, after);
  }
