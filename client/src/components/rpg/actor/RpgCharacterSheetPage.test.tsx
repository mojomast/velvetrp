...
function api(): RpgCharacterSheetApi {
  return { getSheet: vi.fn(), getResources: vi.fn().mockResolvedValue({ resources: [], revision: 4 }), getInventory: vi.fn().mockResolvedValue(inventory), getWallet: vi.fn().mockResolvedValue({ wallet: { balances: [] }, revision: 4 }), getEffects: vi.fn().mockResolvedValue({ effects: [], concentration: [], revision: 4 }), getPowers: vi.fn().mockResolvedValue({ known: [], prepared: [], slots: [], uses: [], legalNow: [], legalCommands: [], revision: 4 }), getShop: vi.fn(), inventoryCommand: vi.fn(), economyCommand: vi.fn(), rest: vi.fn(), checkCommand: vi.fn(), powerCommand: vi.fn(), spellCommand: vi.fn(), effectCommand: vi.fn(), resourceCommand: vi.fn(), vendorSaleQuote: vi.fn(), getCampaignContent: vi.fn().mockRejectedValue(new Error("no catalog")), getCampaignPack: vi.fn() };
}
