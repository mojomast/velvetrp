...
function api(): RpgCharacterSheetApi {
  return { getSheet: vi.fn(), getResources: vi.fn().mockResolvedValue({ resources: [], revision: 4 }), getInventory: vi.fn().mockResolvedValue(inventory), getWallet: vi.fn().mockResolvedValue({ wallet: { balances: [] }, revision: 4 }), getEffects: vi.fn().mockResolvedValue({ effects: [], concentration: [], revision: 4 }), getShop: vi.fn(), inventoryCommand: vi.fn(), economyCommand: vi.fn(), rest: vi.fn(), getCampaignContent: vi.fn().mockRejectedValue(new Error("no catalog")), getCampaignPack: vi.fn() };
}
