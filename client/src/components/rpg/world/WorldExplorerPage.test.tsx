...
describe("embedded travel", () => {
  it("uses inline review, existing mutation recovery, and no standalone chrome", async () => {
    ...
    const api: WorldExplorerApi = { getWorld: vi.fn().mockResolvedValueOnce({ data: world, revision: 2 }).mockResolvedValue({ data: { ...world, currentLocations: [{ actorId: "actor", locationId: "road", revision: 3, updatedAt: at }] }, revision: 3 }), travel: vi.fn().mockResolvedValue({ receipt: { idempotencyKey: "travel", revisionBefore: 2, revisionAfter: 3, occurredAt: at } }), place: vi.fn(), camp: vi.fn() };
