...
export function WorldExplorerPage({ campaignId, authorization, api, onBack, focusHeadingRequest }: { campaignId: string; authorization: StudioAuthorization; api: WorldExplorerApi; onBack: () => void; focusHeadingRequest?: number }) {
  ...
  async function travel(actorId: string, command: ActorTravelCommandRequest) {
    ...
    const pending = beginNarrativeMutation(campaignId, "travel", "Travel", { actorId, command }); if (!pending) return;
    try { const result = await api.travel(actorId, command); markNarrativeConfirmed(pending, result, receiptFrom(result)); const refreshed = await api.getWorld(campaignId).then((next) => { if (mounted.current) { setWorld(next.data); setRevision(next.revision); setPhase("ready"); } return true; }).catch(() => false); if (refreshed) consumeNarrativeConfirmed({ ...pending, phase: "confirmed", result, receipt: receiptFrom(result), refresh: "required" }); else markNarrativePartial({ ...pending, phase: "confirmed", result, receipt: receiptFrom(result), refresh: "required" }); }
