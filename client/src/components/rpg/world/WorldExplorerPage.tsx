...
function WorldExplorerWorkspace({ campaignId, sessionId, authorization, api, startingLocationApi = campaignStartingLocationApi, onBack, focusHeadingRequest, embedded = false, blocked = false, actors, onLockChange, onStateChange }: { campaignId: string; sessionId?: string; authorization: StudioAuthorization; api: WorldExplorerApi; startingLocationApi?: CampaignStartingLocationApi; onBack: () => void; focusHeadingRequest?: number; embedded?: boolean; blocked?: boolean; actors?: readonly { actorId: string; name: string }[]; onLockChange?: (locked: boolean) => void; onStateChange?: () => void }) {
  ...
  const load = useCallback(async (focus = false) => { const token = ++generation.current; setPhase("loading"); try { const next = await api.getWorld(campaignId, sessionId); if (!mounted.current || token !== generation.current) return false; setWorld(next.data); setRevision(next.revision); setPhase("ready"); if (focus) queueMicrotask(() => heading.current?.focus()); return next.revision; } catch (error) { if (mounted.current && token === generation.current) setPhase(error instanceof ApiError && error.code === "RPG_WORLD_NOT_FOUND" ? "empty" : "failed"); return false; } }, [api, campaignId, sessionId]);
