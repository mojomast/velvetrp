...
import { commandQuest, getCampaignStory, getCampaignWorld, listCampaignFactions, listCampaignNpcs, listCampaignQuests, projectFactionsForPlayers, projectNpcsForPlayers, projectQuestsForPlayers, projectStoryForPlayers, travelActor } from "./api";
...
describe("M2.10 world, cast, quest, and story API bindings",()=>{
  ...
  it("requires no-store role-union reads and authoritative revision headers",async()=>{const fetchMock=vi.fn()
    ...
    await getCampaignWorld("campaign");await listCampaignNpcs("campaign");await listCampaignFactions("campaign");await listCampaignQuests("campaign");await getCampaignStory("campaign");
    ...
    vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(JSON.stringify({npcs:[],relationships:[]}),{status:200})));await expect(listCampaignNpcs("campaign")).rejects.toThrow(/x-world-revision/);
  });
