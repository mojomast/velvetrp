...
## Post-refactor audit
- Keep `campaignRepo.ts`: `server/src/repo/index.ts` is the sole root-import call site and preserves the public barrel's historical exports plus deprecated event-read delegates. Retain the delegates until a public-barrel contract test proves external consumers do not rely on their root import path; each delegates to the database-scoped event reader without taking connection ownership.
